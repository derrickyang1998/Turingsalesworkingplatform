...
- [ ] Playwright `webServer` owns fixture start, health wait, and process stop; because Playwright force-terminates process trees on Windows, the outer baseline runner must remove the ignored temp DB/log in `finally` after Playwright exits. Tests wait for `document.fonts.ready` and fail when browser revision, OS, locale, timezone, or required fonts differ from the manifest. / Playwright `webServer` 负责夹具启动、健康等待和进程停止；由于 Playwright 在 Windows 强制结束进程树，外层基线启动器必须在 Playwright 退出后的 `finally` 中清理临时 DB/日志。测试等待字体就绪，并在环境元数据变化时失败。
...
- [ ] Record run labels, environment metadata, both per-PNG SHA-256 values, raw and Playwright-compatible `pixelmatch` (`threshold: 0.2`) differing-pixel counts, maximum channel delta, and perceptual `maxDiffPixelRatio <= 0.005` in `ui-ppt-manifest.json`; also record whether every hash is exactly identical. Promote run B only after file sets match and the perceptual pixel threshold passes. / 在清单记录运行标签、环境、两份逐 PNG 哈希、原始差异像素数、与 Playwright 一致的 `pixelmatch`（阈值 `0.2`）感知差异像素数、最大通道差与感知 `maxDiffPixelRatio <= 0.005`，并同时记录哈希是否完全一致；仅文件集合一致且感知像素阈值通过后提升 B。
