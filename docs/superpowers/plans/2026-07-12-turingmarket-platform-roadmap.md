...
- **EN:** Current accepted production release is `v0.9.17-influencer-tenant-isolation`, accepted source `b25f53361e21acb5c32e2655249098837630cecb`, deployed and verified on 2026-09-18. It preserves the v0.9.16 product surface while upgrading to schema v23 and enforcing organization ownership across influencer read/write, Campaign, collaboration, publication, and Feishu-selection paths. All 4,786 production influencer rows are owned, with zero null or unknown owners. Ordinary work follows one independently useful feature per immediate production release; broad regression remains phase-closeout or risk-triggered.<br>**中文：** 当前已验收生产版本为 `v0.9.17-influencer-tenant-isolation`，接纳源码 `b25f53361e21acb5c32e2655249098837630cecb`，已于 2026-09-18 部署并验收。它保留 v0.9.16 产品能力，升级至 schema v23，并在网红读写、Campaign、合作、发布及飞书选择路径强制组织归属；生产 4,786 条网红全部完成归属，空/未知归属为 0。普通工作继续按一个独立可用功能一个即时生产版本推进，完整回归仅由阶段收口或风险触发。
...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-18）：** `v0.9.2`-`v0.9.16` shipped the central policy, authoritative role facts, CRM/customer-task governance, owner transfer, protected-account controls, and Campaign/customer-report/influencer export-import action permissions. `v0.9.17` upgrades production to schema v23 and adds row-level influencer tenant ownership across M4, Campaign, collaboration, publication, and Feishu-selection paths; all 4,786 production influencer rows are owned with zero null/unknown owners. Remaining module permissions, entitlements, and quotas stay separate. / `v0.9.2` 至 `v0.9.16` 已上线中央策略、权威角色事实、CRM/客户任务治理、所有权转移、受保护账号控制及 Campaign/客户报告/网红导出导入动作权限；`v0.9.17` 将生产升级至 schema v23，并在 M4、Campaign、合作、发布及飞书选择路径实现网红行级租户归属，4,786 条生产网红全部有归属且空/未知归属为 0。其余模块权限、权益及配额继续分离。
