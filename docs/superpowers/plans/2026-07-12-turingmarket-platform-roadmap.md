...
**Latest accepted patch / 最新验收补丁：** `v0.8.20-payment-settlement-checkpoint` adds multi-tranche client receipts and creator payments, append-only void/history evidence, variance controls, active-digest settlement submission, independent four-eyes approval, and an atomic actual-cost projection. Financial evidence is Campaign-linked, reserved against generic ingestion, and excluded from RAG; legacy v1 behavior, the approved M4/PPT interfaces, and production schema `v16` remain compatible. / `v0.8.20-payment-settlement-checkpoint` 新增分笔客户回款与达人付款、只追加冲销/历史凭证、差异控制、有效摘要结算提交、独立四眼审核及实际成本原子投影；财务凭证关联 Campaign、禁止通用入口伪造且排除在 RAG 之外，历史 v1 行为、已批准 M4/PPT 界面和生产 schema `v16` 保持兼容。

**Open boundary / 未完成边界：** Guided import mapping, field search, account-synchronized saved views, column workspace management, collaboration commercial terms, signed-contract/PDF custody, content-review/publication gating, and manual payment/four-eyes settlement are accepted. Phase 7 remains open for e-signature, review attachments/customer portals, richer publication custody, automated payment-provider receipts and reconciliation, campaign review, and real configured Feishu delivery. / 引导式导入映射、字段检索、账号同步保存视图、列工作区、合作商业条款、签约/PDF 保管、内容审核/发布门禁及人工收付款/四眼结算已通过验收；阶段 7 仍需完成电子签约、审核附件/客户门户、更完整发布保管、自动支付回单与对账、项目复盘及真实配置飞书投递。
