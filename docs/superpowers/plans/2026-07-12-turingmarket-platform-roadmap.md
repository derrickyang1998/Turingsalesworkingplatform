...
- **EN:** Current accepted production release is `v0.9.23-plan-catalog-module-entitlements`, accepted implementation HEAD `b9874d59fa5742aaf36569e4f9f328848b401673`, deployed and verified on 2026-09-20. Production runs schema v27 with the earlier tenant-owned influencer, knowledge, AI-conversation, token-usage, and AI-quota controls intact. An immutable plan/module catalog, append-only organization assignment history, and server-side role-permission intersection with live plan entitlements now govern seven current modules. Subscription expiry, organization monthly shared quotas, billing, and persistent concurrency reservations remain separate releases. The latest product shell and frozen PPT are unchanged. Ordinary work follows one independently useful feature per immediate production release; broad regression remains phase-closeout or risk-triggered.<br>**中文：** 当前已验收生产版本为 `v0.9.23-plan-catalog-module-entitlements`，接纳实现提交为 `b9874d59fa5742aaf36569e4f9f328848b401673`，已于 2026-09-20 部署并验收。生产运行 schema v27，并保留此前网红、知识库、AI 对话、Token 用量租户归属和 AI 配额控制；不可变套餐/模块目录、仅追加组织套餐历史及服务端实时角色权限与套餐权益交集现已管控七个当前模块。订阅到期、组织月度共享配额、账单及持久并发预留继续拆分交付；最新产品壳层与冻结 PPT 未变化。普通工作继续按一个独立可用功能一个即时生产版本推进，完整回归仅由阶段收口或风险触发。
...
- [ ] **EN:** Add plan, module entitlement, quota, usage, expiry, disabled-account, password reset, session revoke, and ownership-transfer controls.<br>**中文：** 增加套餐、模块权益、配额、用量、到期、账号禁用、密码重置、会话撤销和所有权转移控制。<br>**Progress / 进展（2026-09-20）：** ownership transfer, protected-account reset, session revocation, tenant-owned immutable usage, server-side AI token quota admission, immutable plan catalog, append-only assignment history, and live module entitlements are in production through `v0.9.23`. Current quota accounting remains user-level `legacy_lifetime`; subscription expiry is next, followed by organization monthly shared quota, persistent concurrency reservation, and billing as separate releases. / 截至 `v0.9.23`，所有权转移、受保护账号重置、会话撤销、租户归属不可篡改用量、服务端 AI 配额准入、不可变套餐目录、仅追加分配历史及实时模块权益均已上线。当前配额仍按用户级 `legacy_lifetime` 计量；下一独立版本为订阅到期，其后依次拆分组织月度共享配额、持久并发预留和账单。
