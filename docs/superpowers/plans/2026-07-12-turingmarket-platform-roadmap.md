...
- **EN:** Current accepted production release is `v0.9.21-ai-token-quota-admission`, accepted implementation HEAD `f1f9c98e0a4ffa85ac875c1ead66e80cb55f1696`, deployed and verified on 2026-09-20. Production remains on schema v26 with influencer, knowledge, AI-conversation, and token-usage tenant ownership. Every trusted DeepSeek path now applies live server-side quota admission and verified provider-usage accounting; Admin quota changes are atomic, projected in the control room, and audited. The current quota period is `legacy_lifetime`; plan catalog, module entitlements, expiry, organization monthly shared quotas, billing, and persistent concurrency reservations remain separate releases. The latest product shell and frozen PPT are unchanged. Ordinary work follows one independently useful feature per immediate production release; broad regression remains phase-closeout or risk-triggered.<br>**中文：** 当前已验收生产版本为 `v0.9.21-ai-token-quota-admission`，接纳实现提交为 `f1f9c98e0a4ffa85ac875c1ead66e80cb55f1696`，已于 2026-09-20 部署并验收。生产继续运行 schema v26，并已具备网红、知识库、AI 对话及 Token 用量租户归属；所有可信 DeepSeek 路径现执行实时服务端配额准入和可信 provider 用量记账，管理员配额更新为原子操作、在控制室投影并写入审计。当前计量周期为 `legacy_lifetime`，套餐目录、模块权益、到期、组织月度共享配额、账单及持久并发预留继续拆分交付；最新产品壳层与冻结 PPT 未变化。普通工作继续按一个独立可用功能一个即时生产版本推进，完整回归仅由阶段收口或风险触发。
...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-20）：** `v0.9.2`-`v0.9.16` shipped the central policy, authoritative role facts, CRM/customer-task governance, owner transfer, protected-account controls, and Campaign/customer-report/influencer export-import action permissions. `v0.9.17`-`v0.9.20` added influencer, knowledge, AI-conversation, and token-usage tenant ownership. `v0.9.21` adds server-owned quota admission on every trusted DeepSeek path. Remaining module permissions and plan entitlements stay separate. / `v0.9.2` 至 `v0.9.16` 已上线中央策略、权威角色事实、CRM/客户任务治理、所有权转移、受保护账号控制及 Campaign/客户报告/网红导出导入动作权限；`v0.9.17` 至 `v0.9.20` 增加网红、知识库、AI 对话及 Token 用量租户归属，`v0.9.21` 为所有可信 DeepSeek 路径增加服务端配额准入；其余模块权限及套餐权益继续分离。
...
- [ ] **EN:** Add plan, module entitlement, quota, usage, expiry, disabled-account, password reset, session revoke, and ownership-transfer controls.<br>**中文：** 增加套餐、模块权益、配额、用量、到期、账号禁用、密码重置、会话撤销和所有权转移控制。<br>**Progress / 进展（2026-09-20）：** ownership transfer, protected-account reset, session revocation on security changes, tenant-owned immutable usage, and server-side AI token quota admission are live. Current quota accounting is user-level `legacy_lifetime`; plan catalog, module entitlement, subscription expiry, organization monthly shared quota, billing, and persistent concurrency reservation are not yet delivered. / 所有权转移、受保护账号重置、安全变更会话撤销、具备租户归属且不可篡改的用量账本及服务端 AI Token 配额准入均已上线；当前为用户级 `legacy_lifetime` 计量，套餐目录、模块权益、订阅到期、组织月度共享配额、账单及持久并发预留尚未交付。
