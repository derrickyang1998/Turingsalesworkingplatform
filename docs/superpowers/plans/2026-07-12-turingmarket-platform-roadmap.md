...
**Latest accepted patch / 最新验收补丁：** `v0.8.18-contract-document-custody` adds permission-scoped PDF upload, immutable document metadata, attachment-only download, active-content rejection, and cryptographic binding between new v2 signed confirmations and same-Campaign contract documents. Contract bytes and generic evidence are excluded from RAG. Historical records and the approved M4/PPT interfaces remain compatible. Production schema is `v16`. / `v0.8.18-contract-document-custody` 新增权限隔离的 PDF 上传、不可变文档元数据、仅附件下载、主动内容拒绝，以及新版 v2 已签确认与同 Campaign 合同文档的密码学绑定；合同原始字节和通用凭证均排除在 RAG 之外。历史记录及已批准 M4/PPT 界面保持兼容，生产 schema 为 `v16`。

**Open boundary / 未完成边界：** Guided import mapping, field search, account-synchronized saved views, column workspace management, collaboration commercial terms, the signed-contract checkpoint, and PDF contract custody are accepted. Phase 7 remains open for e-signature, content review, publish, payment, settlement, review, and real configured Feishu delivery. / 引导式导入映射、字段检索、账号同步保存视图、列工作区、合作商业条款、已签合同关口及 PDF 合同保管已通过验收；阶段 7 仍需完成电子签约、内容审核、发布、付款、结算、复盘及真实配置飞书投递。
