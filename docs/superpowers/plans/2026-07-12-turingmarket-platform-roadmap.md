...
- **EN:** Current accepted production release is `v0.9.18-knowledge-tenant-isolation`, accepted implementation HEAD `4df9104c0c18911a66a51277fd0a774aabe7ff35`, deployed and verified on 2026-09-19. It preserves the v0.9.17 product surface while upgrading to schema v24 and enforcing active-organization ownership across ordinary knowledge ingest/search, AI/RAG, proposal/PPT context, and business-artifact producers. All 128 production knowledge entries are owned, with zero null owners; explicit platform-admin knowledge and AI-conversation audit views remain global. Ordinary work follows one independently useful feature per immediate production release; broad regression remains phase-closeout or risk-triggered.<br>**中文：** 当前已验收生产版本为 `v0.9.18-knowledge-tenant-isolation`，接纳实现提交为 `4df9104c0c18911a66a51277fd0a774aabe7ff35`，已于 2026-09-19 部署并验收。它保留 v0.9.17 产品能力，升级至 schema v24，并在普通知识入库/检索、AI/RAG、方案/PPT 上下文及业务产物生产路径强制活动组织归属；生产 128 条知识全部完成归属且空归属为 0，平台管理员的显式知识与 AI 对话审计界面继续保持全局可见。普通工作继续按一个独立可用功能一个即时生产版本推进，完整回归仅由阶段收口或风险触发。
...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-19）：** `v0.9.2`-`v0.9.16` shipped the central policy, authoritative role facts, CRM/customer-task governance, owner transfer, protected-account controls, and Campaign/customer-report/influencer export-import action permissions. `v0.9.17` added row-level influencer tenant ownership; `v0.9.18` upgrades production to schema v24 and adds knowledge tenant ownership across ingest/search, AI/RAG, proposal/PPT context, and business-artifact paths while preserving explicit global Admin audits. Remaining module permissions, entitlements, and quotas stay separate. / `v0.9.2` 至 `v0.9.16` 已上线中央策略、权威角色事实、CRM/客户任务治理、所有权转移、受保护账号控制及 Campaign/客户报告/网红导出导入动作权限；`v0.9.17` 增加网红行级租户归属，`v0.9.18` 将生产升级至 schema v24，并在知识入库/检索、AI/RAG、方案/PPT 上下文和业务产物路径实现知识租户归属，同时保留显式全局 Admin 审计。其余模块权限、权益及配额继续分离。
- [ ] **EN:** Add organization-scoped customers, campaigns, knowledge, AI runs, influencer selections, orders, workflows, tasks, and exports.<br>**中文：** 客户、活动、知识、AI 运行、网红选择、订单、工作流、任务和导出全部纳入组织范围隔离。<br>**Progress / 进展（2026-09-19）：** Knowledge rows and ordinary knowledge/AI-RAG consumers are now organization-scoped in v0.9.18; AI-conversation ownership migration and the remaining module-by-module isolation work stay separate. / v0.9.18 已完成知识行及普通知识/AI-RAG 消费路径的组织隔离；AI 对话归属迁移及其余模块逐项隔离继续分离推进。
