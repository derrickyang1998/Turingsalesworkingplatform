...
- **EN:** Current accepted production release is `v0.9.24-subscription-expiry-control`, accepted implementation HEAD `6b2d579e1c3c7ceecf76af627d166e7e20a8a961`, deployed and verified on 2026-09-21. Production runs schema v28 with the earlier tenant-owned influencer, knowledge, AI-conversation, token-usage, AI-quota, and live plan-entitlement controls intact. Organization subscription terms are now append-only and server-authoritative; expired organizations immediately lose governed module actions while login, authenticated diagnosis, subscription status, and platform-admin renewal remain available. Organization monthly shared quotas, persistent concurrency reservation, billing, and dynamic catalog editing remain separate releases. The latest product shell and frozen PPT are unchanged. Ordinary work follows one independently useful feature per immediate production release; broad regression remains phase-closeout or risk-triggered.<br>**中文：** 当前已验收生产版本为 `v0.9.24-subscription-expiry-control`，接纳实现提交为 `6b2d579e1c3c7ceecf76af627d166e7e20a8a961`，已于 2026-09-21 部署并验收。生产运行 schema v28，并保留此前网红、知识库、AI 对话、Token 用量租户归属、AI 配额及实时套餐权益控制；组织订阅期限现采用服务端权威、仅追加历史，到期组织会立即失去受管模块动作，同时保留登录、已认证诊断、状态查询和平台管理员续期入口。组织月度共享配额、持久并发预留、账单及动态套餐编辑继续拆分交付；最新产品壳层与冻结 PPT 未变化。普通工作继续按一个独立可用功能一个即时生产版本推进，完整回归仅由阶段收口或风险触发。
...
- [ ] **EN:** Add plan, module entitlement, quota, usage, expiry, disabled-account, password reset, session revoke, and ownership-transfer controls.<br>**中文：** 增加套餐、模块权益、配额、用量、到期、账号禁用、密码重置、会话撤销和所有权转移控制。<br>**Progress / 进展（2026-09-21）：** ownership transfer, protected-account reset, session revocation, tenant-owned immutable usage, server-side AI token quota admission, immutable plan catalog, append-only assignment history, live module entitlements, and append-only server-enforced subscription expiry are in production through `v0.9.24`. Current quota accounting remains user-level `legacy_lifetime`; organization monthly shared quota is next, followed by persistent concurrency reservation and billing as separate releases. / 截至 `v0.9.24`，所有权转移、受保护账号重置、会话撤销、租户归属不可篡改用量、服务端 AI 配额准入、不可变套餐目录、仅追加分配历史、实时模块权益及服务端订阅到期控制均已上线。当前配额仍按用户级 `legacy_lifetime` 计量；下一独立版本为组织月度共享配额，其后依次拆分持久并发预留和账单。
