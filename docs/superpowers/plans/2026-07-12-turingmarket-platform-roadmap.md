...
- **EN:** Current accepted production release is `v0.9.25-organization-monthly-ai-quota`, accepted implementation HEAD `f96313255bf26b724c3ad1432c60eebeb15fa554`, deployed and verified on 2026-09-21. Production runs schema v29 with tenant ownership, plan entitlements, subscription expiry, and personal lifetime quota backstop intact. Organization monthly shared AI quota is now append-only, UTC-calendar-month based, server-authoritative, and enforced before every trusted provider path; administrators can inspect and edit it through audited optimistic concurrency controls. Persistent concurrency reservation, billing, and dynamic catalog editing remain separate releases. The latest product shell and frozen PPT are unchanged. Ordinary work follows one independently useful feature per immediate production release; broad regression remains phase-closeout or risk-triggered.<br>**中文：** 当前已验收生产版本为 `v0.9.25-organization-monthly-ai-quota`，接纳实现提交为 `f96313255bf26b724c3ad1432c60eebeb15fa554`，已于 2026-09-21 部署并验收。生产运行 schema v29，并保留租户归属、套餐权益、订阅到期及个人终身配额兜底；组织月度共享 AI 配额现采用仅追加、UTC 自然月、服务端权威合同，并在所有可信 Provider 路径前执行，管理员可通过审计化乐观并发控制查看和编辑。持久并发预留、账单及动态套餐编辑继续拆分交付；最新产品壳层与冻结 PPT 未变化。普通工作继续按一个独立可用功能一个即时生产版本推进，完整回归仅由阶段收口或风险触发。
...
- [ ] **EN:** Add plan, module entitlement, quota, usage, expiry, disabled-account, password reset, session revoke, and ownership-transfer controls.<br>**中文：** 增加套餐、模块权益、配额、用量、到期、账号禁用、密码重置、会话撤销和所有权转移控制。<br>**Progress / 进展（2026-09-21）：** ownership transfer, protected-account reset, session revocation, tenant-owned immutable usage, server-side AI token quota admission, immutable plan catalog, append-only assignment history, live module entitlements, append-only subscription expiry, and append-only organization monthly shared AI quota are in production through `v0.9.25`. Organization quota uses UTC calendar months and remains layered above the personal `legacy_lifetime` backstop. Persistent concurrency reservation is next, followed by billing as a separate release. / 截至 `v0.9.25`，所有权转移、受保护账号重置、会话撤销、租户归属不可篡改用量、服务端 AI 配额准入、不可变套餐目录、仅追加分配历史、实时模块权益、仅追加订阅到期及组织月度共享 AI 配额均已上线。组织配额按 UTC 自然月计量，并继续叠加个人 `legacy_lifetime` 兜底；下一独立版本为持久并发预留，其后为账单。
