...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-16）：** `v0.9.2` and `v0.9.3` are deployed and verified. Schema v21 now persists company-owner/read-only facts and enforces scoped organization-member governance, session revocation, and a live central read-only write barrier. Module-by-module adoption and ownership transfer remain open. / `v0.9.2` 与 `v0.9.3` 均已部署验收；schema v21 已持久化企业所有者/只读事实并执行组织成员范围治理、会话撤销和实时中央只读写入拦截；逐模块接入和所有权转移仍待完成。
