...
**Latest accepted patch / 最新验收补丁：** `v0.8.21-campaign-closeout-review` adds a human-confirmed M4 closeout review, a permission-scoped server-authoritative collaboration/payment snapshot, `campaign_review` knowledge archival with paired `knowledge`/`review` links, and the final `settled -> reviewed` transition. Current import, Feishu, collaboration, AI/knowledge, approved M4/PPT interfaces, and production schema `v16` remain compatible. / `v0.8.21-campaign-closeout-review` 新增人工确认的 M4 结案复盘、受权限约束的服务端权威合作/收付款快照、带 `knowledge`/`review` 双关联的 `campaign_review` 知识归档，以及最终 `settled -> reviewed` 状态推进；现有导入、飞书、合作、AI/知识、已批准 M4/PPT 界面和生产 schema `v16` 保持兼容。

**Open boundary / 未完成边界：** Guided import mapping, field search, account-synchronized saved views, column workspace management, collaboration commercial terms, signed-contract/PDF custody, content-review/publication gating, manual payment/four-eyes settlement, and campaign closeout review are accepted. Phase 7 remains open for e-signature, review attachments/customer portals, richer publication custody, automated payment-provider receipts and reconciliation, and real configured Feishu delivery. / 引导式导入映射、字段检索、账号同步保存视图、列工作区、合作商业条款、签约/PDF 保管、内容审核/发布门禁、人工收付款/四眼结算及项目结案复盘已通过验收；阶段 7 仍需完成电子签约、审核附件/客户门户、更完整发布保管、自动支付回单与对账及真实配置飞书投递。
...
**Releases / 版本：** `v0.8.1-performance-manual-foundation`, `v0.8.2-performance-collection-feishu`, `v0.8.3-performance-ai-review`; shipped patch slices through `v0.8.25-performance-freshness-update-queue` / 已发布补丁切片至 `v0.8.25-performance-freshness-update-queue`

**Latest accepted slice / 最新验收切片：** `v0.8.25` adds a provider-independent freshness summary and manual update queue to the existing Content Monitor, evaluates the full authorized campaign projection with deterministic cadence and explicit unknown/stale states, and keeps automatic-provider status truthfully unconfigured. Production schema remains `v17`; live Feishu writes and automatic collection remain disabled. / `v0.8.25` 已在既有内容监控增加服务商无关的新鲜度概览与人工更新队列，按完整授权活动投影、确定性频率及明确未知/过期状态计算，并如实保持自动 provider 未配置。生产 schema 保持 `v17`；真实飞书写入和自动采集仍未启用。
