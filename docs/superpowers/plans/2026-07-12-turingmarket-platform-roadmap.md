...
**Latest accepted patch / 最新验收补丁：** `v0.8.19-content-review-checkpoint` adds evidence-backed content submission, independent approval/change requests, complete immutable history, permission capability projection, historical-v2 recovery, and a hard publication gate. Review URLs and notes stay in protected metadata and are excluded from RAG; generic knowledge paths cannot fabricate the reserved namespace. The approved M4/PPT interfaces remain compatible and production schema remains `v16`. / `v0.8.19-content-review-checkpoint` 新增带凭证的内容送审、独立通过/退回、完整不可变历史、权限能力投影、历史 v2 修复入口和发布强制门禁；审核链接与说明只保存在受控元数据中并排除在 RAG 之外，通用知识路径无法伪造保留命名空间。已批准的 M4/PPT 界面保持兼容，生产 schema 仍为 `v16`。

**Open boundary / 未完成边界：** Guided import mapping, field search, account-synchronized saved views, column workspace management, collaboration commercial terms, signed-contract/PDF custody, and the content-review/publication gate are accepted. Phase 7 remains open for e-signature, review attachments/customer portals, richer publication custody, payment, settlement, campaign review, and real configured Feishu delivery. / 引导式导入映射、字段检索、账号同步保存视图、列工作区、合作商业条款、签约/PDF 保管及内容审核/发布门禁已通过验收；阶段 7 仍需完成电子签约、审核附件/客户门户、更完整发布保管、付款、结算、项目复盘及真实配置飞书投递。
