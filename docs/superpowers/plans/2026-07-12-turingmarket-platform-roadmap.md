...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-15）：** `v0.9.2` is deployed and verified in production. It centralizes a fail-closed, live-database `platform_administration.manage` policy and migrates the existing administrator gate; authenticated administrator success and live ordinary-user denial passed online. Company-owner/read-only persistence and module-by-module adoption remain open. / `v0.9.2` 已部署生产并验收，建立默认拒绝、实时数据库驱动的中央权限策略并接管既有管理员入口；管理员成功和实时普通用户拒绝均在线通过，企业所有者/只读持久化及逐模块接入仍待完成。
