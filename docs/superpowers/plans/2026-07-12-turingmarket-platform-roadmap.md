...
**Latest accepted patch / 最新验收补丁：** `v0.8.16-collaboration-commercial-terms` adds an immutable v2 commercial contract for creator cost, client quote, currency, server-derived margin, and payment terms; preserves historical orders through the existing cost projection; and makes mixed-currency summaries explicit instead of adding incompatible amounts. Production schema remains `v15`. / `v0.8.16-collaboration-commercial-terms` 新增不可变 v2 商业条款合同，分别保存网红成本、客户报价、币种、服务端计算毛利和付款条件；通过既有成本投影兼容历史订单，并明确处理多币种汇总，不再直接相加不兼容金额；生产 schema 保持 `v15`。

**Open boundary / 未完成边界：** Guided import mapping, field search, account-synchronized saved views, column workspace management, and collaboration commercial terms are accepted. Phase 7 remains open for real configured Feishu delivery and the complete negotiation, contract, content review, publish, payment, settlement, and review lifecycle. / 引导式导入映射、字段检索、账号同步保存视图、列工作区和合作商业条款已通过验收；阶段 7 仍需完成真实配置飞书投递，以及完整谈判、签约、内容审核、发布、付款、结算与复盘生命周期。
