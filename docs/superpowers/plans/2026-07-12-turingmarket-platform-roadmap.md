...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-17）：** `v0.9.2` and `v0.9.3` established and deployed the central policy and authoritative role-governance facts. `v0.9.4`, `v0.9.5`, and `v0.9.6` then deployed exact tenant-scoped read/create/update policies for CRM customers, opportunities, and contacts, including embedded-detail protection, pre-parser denial audit, permission-aware controls, and stale-detail response protection. Tasks, remaining modules, and ownership transfer remain open. / `v0.9.2` 与 `v0.9.3` 已上线中央策略及权威角色治理事实；`v0.9.4`、`v0.9.5`、`v0.9.6` 已依次上线 CRM 客户、商机和联系人精确读建改及租户范围，并覆盖内嵌详情、解析前拒绝审计、权限感知控件和陈旧详情响应保护；任务、其余模块及所有权转移仍待完成。
