...
- **EN:** Reduce production release latency by reusing a verified parser dependency cache only when dependency lockfiles, trusted verifier identity, platform, and ownership/mode evidence all match; rebuild on any mismatch. Deliver this as a separately reviewed shared-infrastructure slice before relying on it for later feature releases.<br>**中文：** 仅当解析器依赖锁文件、可信校验器身份、平台及属主/权限证据全部一致时复用已验证依赖缓存，任一不一致即重建，以缩短每次生产发布耗时；该能力必须先作为独立的共享基础设施切片完成审查和上线，后续功能发布才能依赖。
...
- **Minimum focused verification / 最小定向验证：** Syntax checks for changed executable files, exact affected unit/API/contract tests, migration idempotence and restore checks when data is involved, focused secret scanning, `git diff --check`, and one independent code-review approval. A UI slice adds only its changed journey at `1440x900` and `390x844` when browser execution is authorized. Heavy full-suite testing is not required for a normal feature slice. / 包括修改后可执行文件的语法检查、精确受影响的单元/API/契约测试、涉及数据时的迁移幂等与恢复检查、定向密钥扫描、`git diff --check` 和一次独立代码审查批准；UI 切片在浏览器执行已获授权时，仅增加改动旅程的 `1440x900` 与 `390x844` 检查。普通功能切片不要求繁重全量测试。
...
- **Delivery tempo / 交付节奏：** A normal feature is deployed to production in the same development round as soon as its minimum focused verification, independent approval, and verified backup are complete. Do not batch unrelated pending features or delay a finished feature merely to wait for a broad regression. The production gate remains mandatory, but the gate input is limited to the named feature and its direct dependencies unless a broad-risk trigger applies. / 普通功能在同一开发轮完成最小定向验证、独立批准和可验证备份后立即部署生产；不得捆绑无关待办，也不得只为等待完整回归而延后已完成功能。生产发布守卫仍是强制项，但除非触发高风险条件，守卫输入只覆盖该功能及其直接依赖。
...
- **EN:** Current accepted production release is `v0.9.26-ai-provider-concurrency-reservation`, accepted implementation HEAD `43bb85b846b50c010f5c14a388950319e0ce59d6`, deployed and verified on 2026-09-21. Production runs schema v30 with tenant ownership, plan entitlements, subscription expiry, organization monthly quota, and personal lifetime quota backstop intact. Every trusted Tavily/DeepSeek operation now acquires one durable organization permit before RAG, web, provider, or business-result writes; one logical operation shares its permit across providers, completed replay uses none, and full capacity fails closed with a typed 429. Administrators use the same versioned limit with no bypass. Billing and dynamic catalog editing remain separate releases. The latest product shell and frozen PPT are unchanged. Ordinary work follows one independently useful feature per immediate production release; broad regression remains phase-closeout or risk-triggered.<br>**中文：** 当前已验收生产版本为 `v0.9.26-ai-provider-concurrency-reservation`，接纳实现提交为 `43bb85b846b50c010f5c14a388950319e0ce59d6`，已于 2026-09-21 部署并验收。生产运行 schema v30，并保留租户归属、套餐权益、订阅到期、组织月度配额及个人终身配额兜底；所有可信 Tavily/DeepSeek 操作现均在 RAG、联网、Provider 或业务结果写入前取得一个组织级持久许可，一次逻辑操作跨 Provider 共享许可，已完成重放不占槽位，满载时类型化 429 失败关闭。管理员使用同一版本化上限且无豁免。账单与动态套餐编辑继续拆分交付；最新产品壳层与冻结 PPT 未变化。普通工作继续按一个独立可用功能一个即时生产版本推进，完整回归仅由阶段收口或风险触发。
...
- [x] **EN:** Add immutable v2 collaboration commercial terms for creator cost, client quote, currency, server-derived margin, and payment terms; preserve historical orders and prevent mixed-currency totals. Production accepted in `v0.8.16` on 2026-09-08.<br>**中文：** 增加不可变 v2 合作商业条款，分别保存网红成本、客户报价、币种、服务端计算毛利和付款条件；兼容历史订单并阻止多币种错误合计；已于 2026-09-08 通过 `v0.8.16` 生产验收。
- [x] **EN:** Add a signed-contract evidence checkpoint with dedicated authorization, idempotent audit, Campaign-linked knowledge evidence, RAG-safe metadata handling, and a hard live-state gate for new v2 orders. Production accepted in `v0.8.17` on 2026-09-08.<br>**中文：** 增加已签合同凭证关口，包含专用授权、幂等审计、Campaign 关联知识凭证、RAG 安全元数据处理，以及新版 v2 订单进入上线状态前的强制门禁；已于 2026-09-08 通过 `v0.8.17` 生产验收。
- [x] **EN:** Add permission-scoped PDF contract upload, immutable append-only custody, active-content rejection, attachment-only download, and same-Campaign document-hash binding for new v2 signed confirmations. Production accepted in `v0.8.18` on 2026-09-08.<br>**中文：** 增加权限隔离的 PDF 合同上传、不可变只追加保管、主动内容拒绝、仅附件下载，以及新版 v2 已签确认与同 Campaign 文档摘要绑定；已于 2026-09-08 通过 `v0.8.18` 生产验收。
- [x] **EN:** Add evidence-backed content submission, independent review, resubmission, complete immutable history, server-projected action permissions, historical-v2 recovery, and a hard publication gate. Production accepted in `v0.8.19` on 2026-09-09.<br>**中文：** 增加带凭证的内容送审、独立审核、重新提交、完整不可变历史、服务端操作权限投影、历史 v2 修复入口及发布强制门禁；已于 2026-09-09 通过 `v0.8.19` 生产验收。
- [x] **EN:** Add multi-tranche client receipts and creator payments, append-only void/history evidence, variance and zero-value controls, active payment digests, independent four-eyes settlement, and an atomic actual-cost projection. Production accepted in `v0.8.20` on 2026-09-09.<br>**中文：** 增加分笔客户回款和达人付款、只追加冲销/历史凭证、差异与零金额控制、有效付款摘要、独立四眼结算及实际成本原子投影；已于 2026-09-09 通过 `v0.8.20` 生产验收。
- [x] **EN:** Add a human-confirmed campaign closeout review backed by a complete server-authoritative collaboration/payment snapshot, archive it as paired knowledge/review evidence, and advance the settled Campaign to reviewed. Production accepted in `v0.8.21` on 2026-09-10.<br>**中文：** 增加由完整服务端权威合作/收付款快照支撑的人工项目结案复盘，将其归档为知识/复盘双关联凭证，并把已结算 Campaign 推进到已复盘；已于 2026-09-10 通过 `v0.8.21` 生产验收。
- [x] **EN:** Add a compact seven-stage collaboration lifecycle rail backed only by existing server evidence, with correct cancelled and legacy-live semantics. Production accepted in `v0.8.27` on 2026-09-11.<br>**中文：** 增加仅由现有服务端凭证驱动的七阶段合作履约轨道，并正确处理取消订单和历史执行订单；已于 2026-09-11 通过 `v0.8.27` 生产验收。
...
- [x] **EN:** Expose provider-independent freshness, coverage, cadence, and an actionable manual update queue from the full authorized campaign projection while automatic collection remains unconfigured. Production accepted in `v0.8.25` on 2026-09-10.<br>**中文：** 在自动采集仍未配置时，基于完整授权活动投影展示服务商无关的新鲜度、覆盖率、频率和可执行人工更新队列；已于 2026-09-10 通过 `v0.8.25` 生产验收。
- [x] **EN:** Expose safe recent update history for current manual and CSV/XLSX flows from existing append-only audit evidence, with campaign authorization, bounded reads, invalid-record handling, and visible truncation. Production accepted in `v0.8.26` on 2026-09-10; provider/scheduler execution remains part of the open item above.<br>**中文：** 基于既有只追加审计证据展示当前人工和 CSV/XLSX 流程的安全最近更新历史，包含活动权限、有界读取、异常记录处理及可见截断提示；已于 2026-09-10 通过 `v0.8.26` 生产验收，provider/调度执行仍归属上方未完成工作项。
...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-20）：** `v0.9.2`-`v0.9.21` shipped the central role/action policy, authoritative role facts, tenant ownership, governed CRM/export/import actions, and AI quota admission. `v0.9.23` adds the immutable catalog and server-side intersection of role permissions with live plan entitlements for CRM customer/opportunity/contact/task, campaign performance/customer report, and influencer data. Additional business modules continue module by module. / `v0.9.2` 至 `v0.9.21` 已上线中央角色/动作策略、权威角色事实、租户归属、受控 CRM/导出/导入动作及 AI 配额准入；`v0.9.23` 新增不可变目录，并对 CRM 客户/商机/联系人/任务、活动效果/客户复盘及网红数据执行服务端角色权限与实时套餐权益交集；其他业务模块继续逐项接入。
- [ ] **EN:** Add organization-scoped customers, campaigns, knowledge, AI runs, influencer selections, orders, workflows, tasks, and exports.<br>**中文：** 客户、活动、知识、AI 运行、网红选择、订单、工作流、任务和导出全部纳入组织范围隔离。<br>**Progress / 进展（2026-09-20）：** Knowledge and ordinary AI/RAG consumers are organization-scoped in v0.9.18; AI conversations are tenant-owned in v0.9.19; token usage is tenant-owned and immutable in v0.9.20. Customers, campaigns, creator selections/orders, workflows, tasks, and remaining exports continue module by module. / v0.9.18 已完成知识库及普通 AI/RAG 消费路径组织隔离，v0.9.19 完成 AI 对话租户归属，v0.9.20 完成 Token 用量租户归属及不可篡改；客户、活动、网红选择/订单、工作流、任务及其余导出继续逐模块推进。
- [ ] **EN:** Add plan, module entitlement, quota, usage, expiry, disabled-account, password reset, session revoke, and ownership-transfer controls.<br>**中文：** 增加套餐、模块权益、配额、用量、到期、账号禁用、密码重置、会话撤销和所有权转移控制。<br>**Progress / 进展（2026-09-21）：** ownership transfer, protected-account reset, session revocation, tenant-owned immutable usage, server-side AI Token admission, immutable plan catalog, append-only assignment and subscription history, organization monthly shared quota, and durable organization provider concurrency are in production through `v0.9.26`. Concurrency uses one durable slot per logical operation, survives process restart, and fails closed before Provider or business writes. Billing is the next independent release, followed by Phase 8 closeout. / 截至 `v0.9.26`，所有权转移、受保护账号重置、会话撤销、租户归属不可篡改用量、服务端 AI Token 准入、不可变套餐目录、仅追加分配与订阅历史、组织月度共享配额及组织级 Provider 持久并发均已上线。并发控制按每次逻辑操作一个持久槽位执行，可跨进程重启，并在 Provider 或业务写入前失败关闭；下一独立版本为账单，其后进行第 8 阶段收口。
- [ ] **EN:** Add a searchable admin control room for users, organizations, roles, AI conversations/runs, knowledge, provider status, import jobs, workflows, audit events, and security events.<br>**中文：** 新增可搜索的管理控制室，覆盖用户、组织、角色、AI 对话/运行、知识、服务状态、导入任务、工作流、审计事件和安全事件。<br>**Progress / 进展（2026-09-20）：** user/organization directory, scoped role management, global AI-conversation audit, global token-usage audit, quota status projection, and audited quota editing are live; provider/import/workflow/security-event consolidation remains open. / 用户/组织目录、范围化角色管理、全局 AI 对话审计、全局 Token 用量审计、配额状态投影及审计化配额编辑均已上线；服务商、导入、工作流及安全事件的统一视图仍待交付。
- [ ] **EN:** Record every privileged admin view/change, including access to another user's AI conversation.<br>**中文：** 记录所有管理员特权查看/变更，包括访问其他用户的 AI 对话。<br>**Progress / 进展（2026-09-20）：** cross-user AI-conversation views, global token-usage reads, role/member administration, protected-account operations, ownership transfer, and quota updates are audited. The remaining privileged surfaces must adopt the same bounded audit contract as they are delivered. / 跨用户 AI 对话查看、全局 Token 用量读取、角色/成员管理、受保护账号操作、所有权转移及配额更新均已审计；其余特权界面随逐项上线继续接入同一字段有界审计合同。
