...
**Latest accepted patch / 最新验收补丁：** `v0.8.17-signed-contract-checkpoint` adds a dedicated signed-contract evidence confirmation, archives immutable Campaign-linked proof with sensitive fields excluded from RAG, and prevents new v2 collaborations from entering `live` before a valid `contracted` checkpoint. Historical orders and the approved M4/PPT interfaces remain compatible. Production schema remains `v15`. / `v0.8.17-signed-contract-checkpoint` 新增专用已签合同凭证确认，将不可变凭证关联 Campaign 归档并排除敏感字段进入 RAG，同时阻止新版 v2 合作在有效 `contracted` 关口前进入 `live`；历史订单及已批准 M4/PPT 界面保持兼容，生产 schema 保持 `v15`。

**Open boundary / 未完成边界：** Guided import mapping, field search, account-synchronized saved views, column workspace management, collaboration commercial terms, and the signed-contract evidence checkpoint are accepted. Phase 7 remains open for contract-file upload or e-signature, content review, publish, payment, settlement, review, and real configured Feishu delivery. / 引导式导入映射、字段检索、账号同步保存视图、列工作区、合作商业条款及已签合同凭证关口已通过验收；阶段 7 仍需完成合同文件上传或电子签约、内容审核、发布、付款、结算、复盘及真实配置飞书投递。
