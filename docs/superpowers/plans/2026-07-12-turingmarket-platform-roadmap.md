...
- **EN:** Current development branch: `codex/v0.5.0-campaign-business-spine`; current production delivery branch remains `codex/v0.4.0-product-shell-and-design-system` until the Phase 4 release gates pass. Each later phase uses the `codex/` prefix and branches from the latest verified production tag or the release branch recorded by the immediately preceding production version, not from a permanently pinned historical branch.<br>**中文：** 当前开发分支为 `codex/v0.5.0-campaign-business-spine`；第 4 阶段发布门禁全部通过前，当前生产交付分支仍为 `codex/v0.4.0-product-shell-and-design-system`。后续每个阶段使用 `codex/` 前缀，并从最新已验证的生产标签或上一生产版本记录的发布分支创建，不得长期固定在历史分支上。
...
- [ ] **EN:** Add provider timeout, bounded retry, cache, circuit-breaker/fallback, token/cost tracking, and safe error messages for DeepSeek and Tavily. Timeout/retry/cache/fallback, safe errors, and bounded token/state projection are production accepted through Task 4D1; versioned model pricing and cost projection remain Task 4D2.<br>**中文：** 为 DeepSeek 和 Tavily 增加超时、有界重试、缓存、熔断/降级、Token/成本跟踪和安全错误信息；超时/重试/缓存/降级、安全错误及有界 Token/状态投影已通过 Task 4D1 生产验收，版本化模型定价与成本投影留在 Task 4D2。
