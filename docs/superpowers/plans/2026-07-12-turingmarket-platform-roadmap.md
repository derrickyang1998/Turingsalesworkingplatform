...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-16）：** `v0.9.2` is deployed and verified. Reviewed candidate `v0.9.3` adds schema-v21 company-owner/read-only persistence, scoped organization-member governance, session revocation, and a live write barrier; guarded production deployment is next. Module-by-module adoption and ownership transfer remain open. / `v0.9.2` 已部署验收；已审查的 `v0.9.3` 候选新增 schema v21 企业所有者/只读持久化、组织成员范围治理、会话撤销和实时写入拦截，下一步为受控生产部署；逐模块接入和所有权转移仍待完成。
