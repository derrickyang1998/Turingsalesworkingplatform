...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-17）：** `v0.9.2` and `v0.9.3` established and deployed the central policy and authoritative role-governance facts. `v0.9.4` deployed `crm.customer` read/create/update with tenant scopes, and `v0.9.5` deployed `crm.opportunity` read/create/update, embedded-detail protection, pre-parser denial audit, and permission-aware opportunity controls. Contacts, tasks, remaining modules, and ownership transfer remain open. / `v0.9.2` 与 `v0.9.3` 已上线中央策略及权威角色治理事实；`v0.9.4` 已上线 CRM 客户读建改与租户范围，`v0.9.5` 已上线 CRM 商机读建改、内嵌详情保护、解析前拒绝审计和权限感知控件；联系人、任务、其余模块及所有权转移仍待完成。
