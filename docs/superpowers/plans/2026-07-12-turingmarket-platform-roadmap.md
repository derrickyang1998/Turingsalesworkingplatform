...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-18）：** `v0.9.2`-`v0.9.6` deployed the central policy, authoritative role facts, and exact tenant-scoped CRM customer/opportunity/contact permissions. `v0.9.7`-`v0.9.10` added task permissions, task workspace, owner transfer, and guarded task reassignment/editing. `v0.9.11` blocks browser reset for protected administrators/owners. `v0.9.12` adds backend-first `campaign.performance.export`, bounded denial audits, and fail-closed UI actions for content and Feishu snapshot CSV. `v0.9.13` separately hardened and accelerated the shared release transport without changing the business surface. Remaining modules, entitlements, and quotas stay separate. / `v0.9.2` 至 `v0.9.6` 已上线中央策略、权威角色事实及 CRM 客户/商机/联系人精确租户权限；`v0.9.7` 至 `v0.9.10` 已增加任务权限、任务工作区、所有权转移和受控任务改派/编辑；`v0.9.11` 上线受保护管理员/所有者密码重置门禁；`v0.9.12` 为内容与飞书快照 CSV 新增后端优先的 `campaign.performance.export`、有界拒绝审计及失败关闭界面；`v0.9.13` 另行加固并加速共享发布传输且不改变业务界面；其余模块、权益及配额继续分离。
