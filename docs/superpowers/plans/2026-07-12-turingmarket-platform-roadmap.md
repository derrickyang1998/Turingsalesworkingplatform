...
- **EN:** Current accepted production release is `v0.9.13-pinned-candidate-bundle-upload`, source `512e50abce569bbf6b0540b606a8e79ddeaa4ae2`, deployed and verified on 2026-09-18. It preserves the v0.9.12 business surface while replacing 403 per-file SSH processes with one deterministic pinned-file stream per attempt. The complete approximately 29.5 MB payload is still transmitted; file-level deduplication is not claimed. Ordinary work follows one independently useful feature per immediate production release; broad regression remains phase-closeout or risk-triggered.<br>**中文：** 当前已验收生产版本为 `v0.9.13-pinned-candidate-bundle-upload`，源码 `512e50abce569bbf6b0540b606a8e79ddeaa4ae2`，已于 2026-09-18 部署并验收。它保持 v0.9.12 的业务界面与能力不变，将 403 个逐文件 SSH 进程替换为每次尝试一个确定性钉住文件流；仍会完整传输约 29.5 MB，不宣称文件级去重。普通工作继续按一个独立可用功能一个即时生产版本推进，完整回归仅由阶段收口或风险触发。
...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-18）：** `v0.9.2`-`v0.9.6` deployed the central policy, authoritative role facts, and exact tenant-scoped CRM customer/opportunity/contact permissions. `v0.9.7`-`v0.9.10` added task permissions, task workspace, owner transfer, and guarded task reassignment/editing. `v0.9.11` blocks browser reset for protected administrators/owners. `v0.9.12` adds backend-first `campaign.performance.export`; `v0.9.13` separately hardened the shared release transport; `v0.9.14` adds backend-first `campaign.customer_report.export`, bounded audit failure handling, and fail-closed HTML/PPTX UI actions. Remaining modules, entitlements, and quotas stay separate. / `v0.9.2` 至 `v0.9.6` 已上线中央策略、权威角色事实及 CRM 客户/商机/联系人精确租户权限；`v0.9.7` 至 `v0.9.10` 已增加任务权限、任务工作区、所有权转移和受控任务改派/编辑；`v0.9.11` 上线受保护管理员/所有者密码重置门禁；`v0.9.12` 增加 `campaign.performance.export`；`v0.9.13` 独立加固共享发布传输；`v0.9.14` 为客户报告 HTML/PPTX 新增后端优先的 `campaign.customer_report.export`、有界审计失败处理及失败关闭界面；其余模块、权益及配额继续分离。
