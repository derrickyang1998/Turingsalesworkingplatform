...
- **EN:** Current accepted production release is `v0.9.15-influencer-data-export-permission`, source `0d9ec5228a5c7cfece3ac31490ca33dc50b8623d`, deployed and verified on 2026-09-18. It preserves the v0.9.14 product surface while adding backend-first `influencer.data.export`, bounded audit, and permission-aware M4 export controls. The existing influencer inventory is a shared global table without `org_id`; row-level tenant isolation remains a separate schema release. Ordinary work follows one independently useful feature per immediate production release; broad regression remains phase-closeout or risk-triggered.<br>**中文：** 当前已验收生产版本为 `v0.9.15-influencer-data-export-permission`，源码 `0d9ec5228a5c7cfece3ac31490ca33dc50b8623d`，已于 2026-09-18 部署并验收。它保留 v0.9.14 产品能力并增加后端优先的 `influencer.data.export`、有界审计及权限感知 M4 导出控件。现有网红主表无 `org_id` 且为全局共享资源，行级租户隔离继续作为独立 schema 版本。普通工作继续按一个独立可用功能一个即时生产版本推进，完整回归仅由阶段收口或风险触发。
...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-18）：** `v0.9.2`-`v0.9.6` deployed the central policy, authoritative role facts, and exact tenant-scoped CRM customer/opportunity/contact permissions. `v0.9.7`-`v0.9.10` added task permissions, task workspace, owner transfer, and guarded task reassignment/editing. `v0.9.11` blocks browser reset for protected administrators/owners. `v0.9.12` adds backend-first `campaign.performance.export`; `v0.9.13` separately hardened the shared release transport; `v0.9.14` adds `campaign.customer_report.export`; `v0.9.15` adds backend-first `influencer.data.export`, bounded deny/success audit, and fail-closed M4 controls. The global influencer table still needs a separate `org_id` migration before row-level tenant isolation can be claimed. Remaining modules, entitlements, and quotas stay separate. / `v0.9.2` 至 `v0.9.6` 已上线中央策略、权威角色事实及 CRM 客户/商机/联系人精确租户权限；`v0.9.7` 至 `v0.9.10` 已增加任务权限、任务工作区、所有权转移和受控任务改派/编辑；`v0.9.11` 上线受保护管理员/所有者密码重置门禁；`v0.9.12` 增加 `campaign.performance.export`；`v0.9.13` 独立加固共享发布传输；`v0.9.14` 增加客户报告导出权限；`v0.9.15` 增加后端优先的 `influencer.data.export`、有界成功/拒绝审计及失败关闭 M4 控件。全局网红主表仍需独立 `org_id` 迁移后才能实现行级租户隔离；其余模块、权益及配额继续分离。
