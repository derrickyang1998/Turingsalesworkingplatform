...
- **EN:** Current accepted production release is `v0.9.12-campaign-performance-export-permission`, feature source `8d074ea17ca4f6d732e702c464f651ff82abc71f`, deployed, recovered, and verified on 2026-09-18. Release-controller source `84ae78f45604982482124d3dbd45b119f90acf48` is pushed for subsequent releases. Ordinary work follows one independently useful feature per immediate production release; broad regression remains phase-closeout or risk-triggered.<br>**中文：** 当前已验收生产版本为 `v0.9.12-campaign-performance-export-permission`，功能源码 `8d074ea17ca4f6d732e702c464f651ff82abc71f`，已于 2026-09-18 部署、恢复并验收；发布控制器源码 `84ae78f45604982482124d3dbd45b119f90acf48` 已推送供后续发布使用。普通工作继续按一个独立可用功能一个即时生产版本推进，完整回归仅由阶段收口或风险触发。
...
- [ ] **EN:** Add explicit module/action permission policies for platform admin, company owner, administrator, manager, member, and read-only roles on top of the Phase 4 stable role codes.<br>**中文：** 在阶段 4 稳定角色编码基础上，为平台管理员、企业所有者、管理员、经理、成员和只读角色增加明确的模块/操作权限策略。<br>**Progress / 进展（2026-09-18）：** `v0.9.2`-`v0.9.6` deployed the central policy, authoritative role facts, and exact tenant-scoped CRM customer/opportunity/contact permissions. `v0.9.7`-`v0.9.10` added task permissions, task workspace, owner transfer, and guarded task reassignment/editing. `v0.9.11` blocks browser reset for protected administrators/owners. `v0.9.12` adds backend-first `campaign.performance.export`, bounded denial audits, and fail-closed UI actions for content and Feishu snapshot CSV. Remaining modules, entitlements, and quotas stay separate. / `v0.9.2` 至 `v0.9.6` 已上线中央策略、权威角色事实及 CRM 客户/商机/联系人精确租户权限；`v0.9.7` 至 `v0.9.10` 已增加任务权限、任务工作区、所有权转移和受控任务改派/编辑；`v0.9.11` 上线受保护管理员/所有者密码重置门禁；`v0.9.12` 为内容与飞书快照 CSV 新增后端优先的 `campaign.performance.export`、有界拒绝审计及失败关闭界面；其余模块、权益及配额继续分离。
