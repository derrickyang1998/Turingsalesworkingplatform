...
**Status / 状态：** Active incremental production delivery. Manual performance, commercial approval, AI review, immutable customer reports/PPT, approved Feishu snapshot export, and the provider-independent freshness update queue are online through `v0.8.25`; scheduled provider collection and live Feishu synchronization remain pending their own gates. / 正在按增量方式持续生产交付。人工效果、商业复核、AI 复盘、不可变客户报告/PPT、已批准飞书快照导出及服务商无关的新鲜度更新队列已上线至 `v0.8.25`；定时 provider 采集与真实飞书同步仍等待各自门禁。

**Target releases / 目标版本：** `v0.8.1-performance-manual-foundation`, `v0.8.2-performance-collection-feishu`, `v0.8.3-performance-ai-review`, with independently deployed production patches through `v0.8.25-performance-freshness-update-queue` / 独立生产补丁已发布至 `v0.8.25-performance-freshness-update-queue`

**Current production checkpoint / 当前生产检查点：** `v0.8.25` adds a campaign-authorized freshness summary and actionable manual update queue to the existing Content Monitor. It uses the full current projection, deterministic cadence, truthful missing/stale states, and explicit `not_configured` provider status. The next gated slice is one explicitly approved provider with scheduler/run history; live Feishu writes remain separate. / `v0.8.25` 已在既有内容监控增加受活动权限约束的新鲜度概览与可执行人工更新队列，使用完整当前投影、确定性频率、真实缺失/过期状态及明确的 provider 未配置状态。下一门禁切片为一个明确批准的 provider 与调度/运行历史；真实飞书写入继续独立交付。
