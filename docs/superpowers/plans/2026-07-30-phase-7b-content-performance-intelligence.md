...
**Status / 状态：** Active incremental production delivery. Manual performance, commercial approval, AI review, immutable customer reports/PPT, approved Feishu snapshot export, provider-independent freshness, and safe collection/update history are online through `v0.8.26`; scheduled provider collection and live Feishu synchronization remain pending their own gates. / 正在按增量方式持续生产交付。人工效果、商业复核、AI 复盘、不可变客户报告/PPT、已批准飞书快照导出、服务商无关新鲜度及安全更新历史已上线至 `v0.8.26`；定时 provider 采集与真实飞书同步仍等待各自门禁。

**Target releases / 目标版本：** `v0.8.1-performance-manual-foundation`, `v0.8.2-performance-collection-feishu`, `v0.8.3-performance-ai-review`, with independently deployed production patches through `v0.8.26-performance-collection-history` / 独立生产补丁已发布至 `v0.8.26-performance-collection-history`

**Current production checkpoint / 当前生产检查点：** `v0.8.26` adds campaign-authorized recent collection/update history for existing manual and CSV/XLSX flows. It uses bounded append-only audit projection, verifies publication scope, rejects malformed/unsupported input, and discloses incomplete history windows without exposing provider diagnostics. The next gated slice is one explicitly approved social-data provider with scheduling and real run execution; live Feishu writes remain separate. / `v0.8.26` 已为既有人工和 CSV/XLSX 流程增加受活动权限约束的最近采集/更新历史，采用有界只追加审计投影，核验内容归属，拒绝异常或不支持输入，并在不暴露平台诊断的前提下披露不完整历史窗口。下一门禁切片为一个明确批准的社交数据 provider、调度及真实运行执行；真实飞书写入继续独立交付。
