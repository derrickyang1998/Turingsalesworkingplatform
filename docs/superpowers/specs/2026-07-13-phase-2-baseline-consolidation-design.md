...
The fixture server lifecycle is owned by Playwright `webServer`: create a temp DB under ignored test output, start the real Express app on a reserved local port, wait for `/api/health`, run the fixture-only browser pass, stop the child process, then remove the temp DB and server log. Two clean captures are written to separate ignored run directories and compared by `platform/server/scripts/compare_ui_baseline_runs.js`; only matching PNGs are promoted to the committed screenshot directory. Run labels, environment metadata, per-file hashes, and the comparison result are recorded in the manifest. / 夹具服务器由 Playwright `webServer` 管理：在忽略目录创建临时数据库、在保留本地端口启动真实 Express、等待健康检查、执行夹具浏览器测试、停止子进程，再清理临时数据库和日志。两次干净采集写入不同忽略目录，由比较脚本逐文件校验；仅完全一致的 PNG 才进入提交目录。运行标签、环境信息、逐文件哈希和比较结果写入清单。
