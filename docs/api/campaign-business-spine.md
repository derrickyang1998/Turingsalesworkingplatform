...
Optional query pairs `active_limit/active_offset`, `history_limit/history_offset`, and `event_limit/event_offset` page the three evidence streams independently. Limits default to 50 and are bounded to 1-100; offsets default to 0. Returns exactly `{ "campaign": Campaign, "active_links": { "demand": WorkspaceLink[], "proposal": WorkspaceLink[], "ppt": WorkspaceLink[], "shortlist": WorkspaceLink[], "order": WorkspaceLink[], "execution": WorkspaceLink[], "publication": WorkspaceLink[], "settlement": WorkspaceLink[], "workflow": WorkspaceLink[], "ai_run": WorkspaceLink[], "knowledge": WorkspaceLink[], "review": WorkspaceLink[] }, "link_history": WorkspaceLink[], "events": EventSummary[], "workflow_dispatches": WorkflowDispatchSummary[], "pagination": { "active_links": { "total": 3, "limit": 50, "offset": 0 }, "link_history": { "total": 0, "limit": 50, "offset": 0 }, "events": { "total": 1, "limit": 50, "offset": 0 } } }`. Totals count immutable evidence rows before redaction; restricted rows within a page remain aggregated by relation. Restricted/missing rows use the tagged redaction contract above. The `knowledge` panel is available to ordinary campaign readers/writers and supports permission-filtered search, open, create/upload/ingest, attach/correct, use, and “use in AI”; it never requires the administrator-only `/kb` route.
