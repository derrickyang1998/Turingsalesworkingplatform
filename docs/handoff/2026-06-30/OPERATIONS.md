...
App cache: 20260714v040productshelldesignsystem
...
Each release creates `/root/turingmarket/backups/v040-product-shell-design-system-<timestamp>` outside the active platform tree. It refuses to reuse an existing path and contains:
...
After backup, files are uploaded only to an isolated `/var/lib/turingmarket-gate/releases/v040-product-shell-design-system-<timestamp>` candidate. The root-only upload manifest is verified before and after locked dependency staging. Root reconstructs an exact schema-only database from the consistent backup and inserts fixed synthetic sentinels; the no-login gate user receives only that sanitized database, runs candidate migration and the full Node suite without external networking, verifies fingerprint and sentinel preservation, runs Playwright 1.61.1 with the Chromium sandbox, and validates Nginx with writable test paths. The active tree, production rows, credentials, session tokens, and production secrets remain unavailable to candidate code throughout these gates. / 备份后所有文件只进入外置隔离候选目录，仅 root 可读的上传清单会在锁定依赖准备前后各校验一次。root 从一致性备份中仅重建完全一致的数据库结构并写入固定合成哨兵；禁止登录的门禁用户只能获得该脱敏数据库，在断网环境执行候选迁移和完整 Node，验证结构指纹与哨兵保持完整，再运行 Playwright 1.61.1 Chromium 沙箱冒烟和可写测试路径 Nginx 校验。门禁全过程中，候选代码均无法访问活动目录、生产业务行、凭据、会话令牌和生产密钥。
