...
- Current accepted production source / 当前已验收生产源码：`0d9ec5228a5c7cfece3ac31490ca33dc50b8623d` (`v0.9.15-influencer-data-export-permission`)
- Current business-feature source / 当前业务功能源码：`0d9ec5228a5c7cfece3ac31490ca33dc50b8623d` (`v0.9.15-influencer-data-export-permission`); it runs on the v0.9.13 single-stream release controller without replacing the accepted product shell / 该业务版本沿用 v0.9.13 单连接发布控制器，不替换已验收产品壳层
...
- Release / 版本：`v0.9.15-influencer-data-export-permission`, deployed and verified on `2026-09-18` / 已于 `2026-09-18` 部署并验收。
- Production run / 生产运行：`7148a3b27fbd49dcb982e3733d9b03db`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260918-164312`；candidate SHA-256 / 候选摘要：`5d9e883760de2be4e5cb66f0e2f26959bdd9f475e4f610d9ee21a68a35d4ebe3`。
...
- Phase 8 current slices / 第 8 阶段当前切片：the fail-closed module/action foundation, authoritative company-owner/read-only facts, scoped member governance, live read-only barrier, four CRM permission families, customer task workspace, task reassignment/editing, atomic organization-owner transfer, protected-account reset, campaign-performance export, customer-report export, and influencer-data export authorizations are active. Remaining modules stay separate. / 默认拒绝权限底座、企业所有者/只读权威事实、成员治理、实时只读拦截、CRM 四类权限、客户任务工作区、任务改派/编辑、原子化组织所有权转移、受保护账号重置、Campaign 效果导出、客户报告导出及网红数据导出权限均已上线；其余模块继续分离。
- Online authorization / 线上权限：`influencer.data.export` is enforced before all, filtered, and selected influencer CSV delivery and projected through login plus `/api/auth/me`. Production acceptance verified an empty selected export, bounded success audit, three enabled administrator controls, logout, and token revocation. The existing influencer inventory is global and has no `org_id`; this release gates the actor action and does not claim row-level tenant isolation. / 网红 CSV 三种模式在交付前强制执行 `influencer.data.export` 并由登录与 `/api/auth/me` 投影；生产验收覆盖空选中导出、成功审计、三个管理员控件、注销及令牌失效。现有网红主表无 `org_id`，本版仅限制操作者动作，不宣称行级租户隔离。
...
- v0.9.15 recovery note / v0.9.15 恢复说明：the first SOCKS/SSH disconnect happened before production mutation and recovery cleaned only the candidate. After enabling SSH keepalives, the accepted run disconnected at `release-replay-complete`; recovery observed that cutover had completed, kept the new release, finalized PM2/Nginx/public-guard acceptance, and removed the lock. The final accepted marker, backup manifests, public health, runtime hashes, database integrity, and online feature path were independently rechecked. / 首次断线在生产变更前仅清理候选；第二次断线发生在远端切换完成后，恢复控制器保留新版本并完成最终化，最终接纳标记、备份、公网、运行时摘要、数据库和线上功能均已独立复核。
...
- Full evidence / 完整证据：`docs/version-records/2026-09-18-v0.9.15-influencer-data-export-permission-production.md`。
...
Production runs accepted v0.9.15 on schema v22. The release preserves the accepted v0.6 product shell, all v0.9.14 capabilities, and frozen PPT renderer while adding the bounded influencer-data export actor permission. The global influencer table still requires a separate `org_id` schema release for row-level tenant isolation. Subsequent ordinary slices use affected tests plus one independent review, verified backup, immediate deployment, and online feature smoke; heavy full-suite testing is reserved for phase closeout or authentication, authorization architecture, schema, shared infrastructure, external-write, and other broad-risk changes. / 生产现运行已验收的 v0.9.15/schema v22，保留既有产品壳层、v0.9.14 全部能力与冻结 PPT，并增加有界网红数据导出操作者权限；行级租户隔离仍需独立 `org_id` schema 版本。后续普通切片继续采用受影响测试、一次独立审查、可验证备份、当轮部署和线上功能冒烟。
