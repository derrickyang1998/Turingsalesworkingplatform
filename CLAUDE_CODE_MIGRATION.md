...
- Current accepted production source / 当前已验收生产源码：`43bb85b846b50c010f5c14a388950319e0ce59d6` (`v0.9.26-ai-provider-concurrency-reservation`)
...
- Current release-controller source / 当前发布控制器源码：`43bb85b` (schema-v30 trusted inventory, provider-concurrency candidate and production acceptance gates, normalized source bytes, and verified interruption-recoverable retention / schema v30 可信清单、Provider 并发候选与生产验收门禁、源码字节规范化及可验证可中断恢复的保留控制)
...
- Release / 版本：`v0.9.26-ai-provider-concurrency-reservation`, deployed and verified on `2026-09-21` / 已于 `2026-09-21` 部署并验收。
- Production run / 生产运行：`fe52e805ed4b445db80da1a5b2a87c49`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260921-085332`；candidate SHA-256 / 候选摘要：`d6afc607a7f5afe2ba28d57f970793e36f318603ea87f5537b669b64e2e61e26`。
...
- Phase 8 current slices / 第 8 阶段当前切片：the fail-closed module/action foundation, authoritative roles, scoped member governance, tenant ownership, server-side AI quota admission, immutable plan entitlements, subscription expiry, organization monthly shared quota, and durable provider concurrency are active. Billing remains the next separate release, followed by Phase 8 closeout. / 默认拒绝权限底座、权威角色、成员治理、租户归属、服务端 AI 配额准入、不可变套餐权益、订阅到期、组织月度共享配额及持久 Provider 并发均已上线；账单为下一独立版本，其后进行第 8 阶段收口。
- Online authorization / 线上权限：ordinary knowledge, AI/RAG, proposal/PPT context, and business-artifact paths require the active authenticated organization. Ordinary users can access only their own organization-scoped conversations; platform administrators retain global knowledge and AI-conversation visibility only through explicit Admin audit views. Every trusted Tavily/DeepSeek operation passes live quota admission and durable organization concurrency before external dispatch; linked completed replays remain idempotent and consume no slot. / 普通知识、AI/RAG、方案/PPT 上下文及业务产物均强制活动认证组织；普通用户只能访问当前组织内自己的对话，平台管理员仅在显式管理审计界面保留全局知识与 AI 对话可见性。所有可信 Tavily/DeepSeek 操作均在外部调用前通过实时配额与组织级持久并发准入，已完成链接会话保持幂等且不占槽位。
...
- v0.9.23 deployment note / v0.9.23 发布说明：the accepted run `53b27349f77045ceb5f0a2773339a7c8` deployed schema v27 and server-enforced plan entitlements. Production acceptance switched organization 1 from `legacy_full v1` to `crm_core v2` and back to `legacy_full v3`, proved all four CRM modules remained available and three excluded modules failed closed, verified two exact audit events, then logged out with zero active sessions. / 正式运行上线 schema v27 与服务端套餐权益；线上将组织 1 从 `legacy_full v1` 切换至 `crm_core v2` 再恢复 `legacy_full v3`，验证 CRM 四模块保留、三项排除模块失败关闭、恰好两条审计记录及验收后零活动会话。
- v0.9.24 deployment note / v0.9.24 发布说明：accepted run `3140a10579f74540b3c99000163a0c09` deployed schema v28 and append-only server-enforced subscription expiry. Online acceptance proved expired organizations lose governed actions and then restored the exact original term. / 正式运行上线 schema v28 与仅追加的服务端订阅到期控制；线上验收验证到期组织失去受管动作并精确恢复原期限。
- v0.9.25 deployment note / v0.9.25 发布说明：accepted run `762b5ca487234ffb80a016f081c17264` deployed schema v29 and append-only organization monthly shared AI quota. Acceptance marker `v0.9.25-20260920204849` proved unlimited v1 -> disabled v2 -> provider-preflight `429 AI_ORGANIZATION_QUOTA_DISABLED` -> exact unlimited v3, with zero conversation, knowledge, token-usage, or web-cache writes during denial and exact identity restoration from the verified backup. The database rejected an attempted append-only audit cleanup; no audit rows were deleted. / 正式运行上线 schema v29 与仅追加的组织月度共享 AI 配额；线上可逆验收验证不限到停用、Provider 前 429 拒绝及精确恢复，拒绝期间无业务副作用，验收身份从校验备份精确恢复；数据库拒绝了对仅追加审计的清理尝试，未删除任何审计记录。
- v0.9.26 deployment note / v0.9.26 发布说明：accepted run `fe52e805ed4b445db80da1a5b2a87c49` deployed schema v30 and durable organization provider concurrency. Production acceptance set a temporary limit of 1, proved a second authenticated AI request failed before provider and business writes, recorded the exact four-event terminal lifecycle, then restored limit 10 with zero active reservations. Root-only evidence SHA-256 is `e1bfb38ddc7e7418daf585c13cbee2c5c366c976700ba9f834fef1b64060fed8`. The requested `derrick` administrator credential was separately restored after a protected backup and audited session revocation; login, identity, logout, and zero final sessions passed. / 正式运行上线 schema v30 与组织级持久 Provider 并发；线上验收临时设置上限 1，验证第二个认证 AI 请求在 Provider 和业务写入前拒绝，记录精确四事件终态链后恢复上限 10 且活动预留为 0。管理员凭据另在受保护备份和审计化会话撤销后恢复，登录、身份、退出及最终零会话均通过。
- Credential protection invariant / 凭据保护不变量：release acceptance must not use `derrick` or any protected owner account and must never turn a failed login into a credential reset. The guarded cutover compares the cutover `security-overlay.json` with the live user identity set and every password hash after mutable acceptance and again before final public acceptance; drift fails closed. Only a current explicit owner instruction permits audited break-glass restoration. / 发布验收禁止使用 `derrick` 或任何受保护所有者账号，登录失败也绝不能转化为凭据重置；受控切换会在可变业务验收后及公网最终验收前两次比对安全覆盖层、用户身份集合与全部密码哈希，漂移即失败关闭；只有所有者当次明确指令才允许审计化应急恢复。
- Delivery cadence / 交付节奏：ordinary features use exact affected tests and one independent review, then verified backup, immediate production deployment, and focused online acceptance in the same round. Authentication, authorization, schema, tenant authority, shared deployment infrastructure, and real external writes trigger broader gates. / 普通功能只跑精确受影响测试与一次独立审查，同轮完成可验证备份、立即部署及线上定向验收；认证、授权、schema、租户权威、共享部署基础设施和真实外部写入触发扩展门禁。
- Full evidence / 完整证据：`docs/version-records/2026-09-21-v0.9.26-ai-provider-concurrency-reservation-production.md`。
...
Production runs accepted v0.9.26 on schema v30. It preserves the accepted product shell and all prior tenant, entitlement, expiry, and monthly quota controls while adding durable organization provider reservations, restart-safe ownership, bounded deadlines, typed fail-closed admission, and audited admin editing. Subsequent ordinary slices use affected tests plus one independent review, verified backup, immediate deployment, and online feature smoke; heavy full-suite testing remains phase-closeout or risk-triggered. / 生产现运行已验收的 v0.9.26/schema v30，保留产品壳层及此前全部租户、权益、到期与月度配额控制，并新增组织级持久 Provider 预留、跨重启所有权、有界期限、类型化失败关闭准入及审计化管理编辑。后续普通切片继续采用受影响测试、一次独立审查、可验证备份、当轮部署和线上功能冒烟；重型全量测试保留到阶段收口或风险触发。
