...
- Current accepted production source / 当前已验收生产源码：`579b019521b9b8b7fdba17d864ff7453510185bd` (`v0.9.14-customer-report-export-permission`)
- Current business-feature source / 当前业务功能源码：`579b019521b9b8b7fdba17d864ff7453510185bd` (`v0.9.14-customer-report-export-permission`); it runs on the v0.9.13 single-stream release controller without replacing the accepted product shell / 该业务版本沿用 v0.9.13 单连接发布控制器，不替换已验收产品壳层
...
- Release / 版本：`v0.9.14-customer-report-export-permission`, deployed and verified on `2026-09-18` / 已于 `2026-09-18` 部署并验收。
- Production run / 生产运行：`a3c5cc049e3b4e2995e7390ea2195dc9`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260918-144928`；candidate SHA-256 / 候选摘要：`7610719e5c17f777558bf43856579c73528f481625c6bf0b971009f5966c9d18`。
...
- Online authorization / 线上权限：`campaign.customer_report.export` is enforced before customer-report HTML/PPTX delivery and projected through login plus `/api/auth/me`. The production tenant currently has no report snapshots, so the authorized path was accepted at the existing ownership-service `404` boundary without fabricating data; read-only requests fail earlier at the global `ORGANIZATION_READ_ONLY` guard with no attachment bytes. / 客户报告 HTML/PPTX 在交付前强制执行 `campaign.customer_report.export` 并由登录与 `/api/auth/me` 投影；生产当前没有报告快照，因此授权路径在既有归属服务 404 边界完成验收且未伪造数据，只读请求则更早由全局只读门禁无附件拒绝。
...
- Full evidence / 完整证据：`docs/version-records/2026-09-18-v0.9.14-customer-report-export-permission-production.md`。
...
Production runs accepted v0.9.14 on schema v22. The release preserves the accepted v0.6 product shell, all v0.9.13 capabilities, and frozen PPT renderer while adding the bounded customer-report export permission. Subsequent ordinary slices use affected tests plus one independent review, verified backup, immediate deployment, and online feature smoke; heavy full-suite testing is reserved for phase closeout or authentication, authorization architecture, schema, shared infrastructure, external-write, and other broad-risk changes. / 生产现运行已验收的 v0.9.14/schema v22，保留既有产品壳层、v0.9.13 全部能力与冻结 PPT，并增加有界客户报告导出权限；后续普通切片采用受影响测试、一次独立审查、可验证备份、当轮部署和线上功能冒烟，繁重全量测试仅用于阶段收口或认证、授权架构、schema、共享基础设施、外部写入等高风险边界。
