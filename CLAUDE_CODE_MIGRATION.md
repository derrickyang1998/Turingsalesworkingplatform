...
- Current accepted production source / 当前已验收生产源码：`f96313255bf26b724c3ad1432c60eebeb15fa554` (`v0.9.25-organization-monthly-ai-quota`)
- Current business-feature source / 当前业务功能源码：implementation range `dbbceb3` through `f963132`; token-usage tenant ownership is in `b4d6ba0`, unified AI quota admission/accounting is in `f1f9c98`, plan entitlements are in `1509d5b` through `b9874d5`, subscription expiry is accepted through `6b2d579`, and organization monthly shared AI quota is in `f963132` / 业务实现范围为 `dbbceb3` 至 `f963132`；Token 用量租户归属位于 `b4d6ba0`，统一 AI 配额准入位于 `f1f9c98`，套餐权益位于 `1509d5b` 至 `b9874d5`，订阅到期控制接纳至 `6b2d579`，组织月度共享 AI 配额位于 `f963132`
- Current release-controller source / 当前发布控制器源码：`f963132` (schema-v29 trusted inventory, organization-quota candidate gate, normalized source bytes, and verified interruption-recoverable retention / schema v29 可信清单、组织配额候选门禁、源码字节规范化及可验证可中断恢复的保留控制)
...
- Release / 版本：`v0.9.25-organization-monthly-ai-quota`, deployed and verified on `2026-09-21` / 已于 `2026-09-21` 部署并验收。
- Production run / 生产运行：`762b5ca487234ffb80a016f081c17264`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260921-042129`；candidate SHA-256 / 候选摘要：`2fd09a74c69b274578f931852bdbf96520abd07ad0b3855318ea46092cf66108`。
- Acceptance-time runtime / 验收时运行状态：PM2 `online`, restart count `0`; Nginx is active; public `/api/health` is `200` with parser ready; schema v29, SQLite `quick_check=ok`, foreign-key violations `0`, focused regression `44/44`, release inventory `21/21`, trusted-source constants `4/4`, and deployment Chromium smoke `3/3` passed / PM2 在线且无重启，Nginx、公网健康、解析器、schema v29、SQLite 完整性、定向回归、发布清单、可信固定值及部署浏览器冒烟均正常。
...
- Database / 数据库：schema `v29`, integrity `ok`, foreign-key violations `0`; immutable plan/module catalog, append-only subscription terms, append-only organization monthly quota policy, and the tenant-owned token ledger are active / schema v29、完整性与外键正常；不可变套餐/模块目录、仅追加订阅期限、仅追加组织月度配额策略及租户归属 Token 账本均已启用。
- Phase 8 current slices / 第 8 阶段当前切片：the fail-closed module/action foundation, authoritative roles, scoped member governance, tenant ownership, server-side AI quota admission, immutable plan entitlements, subscription expiry, and organization monthly shared quota are active. Persistent concurrency reservation and billing remain separate releases. / 默认拒绝权限底座、权威角色、成员治理、租户归属、服务端 AI 配额准入、不可变套餐权益、订阅到期及组织月度共享配额均已上线；持久并发预留和账单继续拆分交付。
...
- Full evidence / 完整证据：`docs/version-records/2026-09-21-v0.9.25-organization-monthly-ai-quota-production.md`。
...
Production runs accepted v0.9.25 on schema v29. It preserves the accepted product shell and all prior tenant/entitlement/expiry controls while adding append-only organization monthly shared AI quota, UTC calendar-month usage, server-side admission before every trusted provider path, and audited admin editing. Subsequent ordinary slices use affected tests plus one independent review, verified backup, immediate deployment, and online feature smoke; heavy full-suite testing remains phase-closeout or risk-triggered. / 生产现运行已验收的 v0.9.25/schema v29，保留产品壳层及此前全部租户、权益和到期控制，并新增仅追加的组织月度共享 AI 配额、UTC 自然月用量、所有可信 Provider 路径前置服务端准入及审计化管理编辑。后续普通切片继续采用受影响测试、一次独立审查、可验证备份、当轮部署和线上功能冒烟；重型全量测试保留到阶段收口或风险触发。
