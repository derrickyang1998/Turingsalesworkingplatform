...
- Current production source / 当前生产源码：`70a03844ad1d7cee9bd821d368521cb1d9ec1413` (`v0.9.11-protected-account-reset`)
...
- Release / 版本：`v0.9.11-protected-account-reset`, deployed and verified on `2026-09-18` / 已于 `2026-09-18` 部署并验收。
- Production run / 生产运行：`671ac27b99a3468daf774772c91d30da`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260918-073131`；candidate SHA-256 / 候选摘要：`2fbb58b1064807113261cc92dbeb8e449f7ccc34fa4b0b75f0339d4270dd9461`。
- Acceptance-time runtime / 验收时运行状态：PM2 `online`, restart count `0`, PID `711210`; Nginx and `pm2-root` `active`; public `/api/health`, `/m0`, `/m0-detail`, and Admin `200`; parser ready; the focused production Chromium journey passed at desktop and mobile sizes / PM2 在线且无重启，Nginx、pm2-root、公网健康、客户双页面、管理端、解析器及本功能桌面/手机浏览器验收正常。
...
- Phase 8 current slices / 第 8 阶段当前切片：the fail-closed module/action foundation, authoritative company-owner/read-only facts, scoped member governance, live read-only barrier, four CRM permission families, customer task workspace, task reassignment/editing, atomic organization-owner transfer, and protected-account reset gate are active. Remaining modules stay separate. / 默认拒绝权限底座、企业所有者/只读权威事实、成员治理、实时只读拦截、CRM 四类权限、客户任务工作区、任务改派/编辑、原子化组织所有权转移及受保护账号重置门禁均已上线；其余模块继续分离。
- Online authorization / 线上权限：browser Admin reset now rejects platform administrators and current company owners through a stable break-glass error without changing password hashes, sessions, or success audits; bounded denial evidence is retained, focused desktop/mobile UI acceptance passed, and active sessions are `0`. / 浏览器管理端密码重置现以稳定 break-glass 错误拒绝平台管理员和当前企业所有者，密码哈希、会话及成功审计不变，保留有界拒绝证据，桌面/手机定向验收通过，活动会话为 0。
...
- Full evidence / 完整证据：`docs/version-records/2026-09-18-v0.9.11-protected-account-reset-production.md`。
