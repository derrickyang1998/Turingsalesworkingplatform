...
- Current accepted production source / 当前已验收生产源码：`6266a2818b9c805ccc2e0135d79b3c94413221db` (`v0.9.19-ai-conversation-tenant-ownership`)
- Current business-feature source / 当前业务功能源码：implementation range `dbbceb3` through `6266a28`; AI-conversation schema/runtime ownership is in `08c1f86`, followed by the exact schema-v25 replay assertion in `6266a28` / 业务实现范围为 `dbbceb3` 至 `6266a28`，AI 对话 schema/运行时归属实现位于 `08c1f86`，其后由 `6266a28` 精确更新 schema v25 重放断言
- Current release-controller source / 当前发布控制器源码：`6266a2818b9c805ccc2e0135d79b3c94413221db` (schema-v25 trusted release inventory and replay compatibility / schema v25 可信发布清单与重放兼容)
...
- Release / 版本：`v0.9.19-ai-conversation-tenant-ownership`, deployed and verified on `2026-09-19` / 已于 `2026-09-19` 部署并验收。
- Production run / 生产运行：`b4fddb142b9b469f9c734a956009f900`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260919-040753`；candidate SHA-256 / 候选摘要：`cbf6e9b3c483d26212fa10bdd45c9150664de787ae82e2a74eccd4dc3f90006e`。
- Acceptance-time runtime / 验收时运行状态：PM2 `online`, restart count `0`; Nginx is valid; public `/api/health` is `200` with parser ready; schema v25, SQLite integrity `ok`, foreign-key violations `0`, remote candidate tests `76/76`, and deployment Chromium smoke `2/2` passed / PM2 在线且无重启，Nginx、公网健康、解析器、schema v25、SQLite 完整性、远端候选测试和部署浏览器冒烟均正常。
...
- Database / 数据库：schema `v25`, integrity `ok`, foreign-key violations `0`; all `34` AI conversations belong to organization `1`, null owners are `0`, all `68` messages match their conversation owner, all `257` references are valid, and two organization-aware indexes plus thirteen custody triggers are present / schema v25、完整性与外键正常；34 个 AI 对话全部归属组织 1，空归属为 0，68 条消息归属一致，257 条引用有效，两个组织索引与十三个保管触发器齐全。
- Phase 8 current slices / 第 8 阶段当前切片：the fail-closed module/action foundation, authoritative company-owner/read-only facts, scoped member governance, live read-only barrier, four CRM permission families, customer task workspace, task reassignment/editing, atomic organization-owner transfer, protected-account reset, campaign-performance export, customer-report export, influencer-data export/import authorizations, influencer row-level tenant isolation, knowledge tenant isolation, and AI-conversation tenant ownership are active. Remaining modules stay separate. / 默认拒绝权限底座、企业所有者/只读权威事实、成员治理、CRM 权限与任务工作区、所有权转移、受保护账号重置、三类导出/导入权限、网红/知识库租户隔离及 AI 对话租户归属均已上线；其余模块继续分离。
- Online authorization / 线上权限：ordinary knowledge, AI/RAG, proposal/PPT context, and business-artifact paths require the active authenticated organization. Ordinary users can access only their own organization-scoped conversations; platform administrators retain global knowledge and AI-conversation visibility only through explicit Admin audit views, while AI generation remains organization-scoped. / 普通知识、AI/RAG、方案/PPT 上下文及业务产物均强制活动认证组织；普通用户只能访问当前组织内自己的对话，平台管理员仅在显式管理审计界面保留全局知识与 AI 对话可见性，AI 生成仍按组织隔离。
...
- v0.9.19 deployment note / v0.9.19 发布说明：the first candidate stopped before production mutation because the replay test still expected schema versions only through v24. The exact one-line assertion update passed independent re-review and the full replay file `8/8`; run `b4fddb142b9b469f9c734a956009f900` then completed with `DEPLOY_OK` and `RETENTION_CLEANUP_OK`. / 首次候选因重放测试仍仅期望 schema 到 v24 而在生产变更前停止；一行精确断言更新通过独立复审和完整重放 `8/8` 后，正式运行成功返回 `DEPLOY_OK` 与 `RETENTION_CLEANUP_OK`。
...
- Full evidence / 完整证据：`docs/version-records/2026-09-19-v0.9.19-ai-conversation-tenant-ownership-production.md`。
...
Production runs accepted v0.9.19 on schema v25. It preserves the accepted product shell, all v0.9.18 capabilities, and frozen PPT renderer while enforcing immutable organization ownership for AI conversations, messages, knowledge/web references, Campaign links, and archive/promotion paths; explicit Admin global audit remains available only through an auditable query. Subsequent ordinary slices use affected tests plus one independent review, verified backup, immediate deployment, and online feature smoke; heavy full-suite testing remains phase-closeout or risk-triggered. / 生产现运行已验收的 v0.9.19/schema v25，保留既有产品壳层、v0.9.18 全部能力与冻结 PPT，并对 AI 对话、消息、知识/联网引用、Campaign 关联及归档路径执行不可变组织归属；管理员全局审计仅可通过显式且可审计的查询进入。后续普通切片继续采用受影响测试、一次独立审查、可验证备份、当轮部署和线上功能冒烟。
