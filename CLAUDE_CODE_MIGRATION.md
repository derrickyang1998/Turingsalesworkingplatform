...
- Current accepted production source / 当前已验收生产源码：`4df9104c0c18911a66a51277fd0a774aabe7ff35` (`v0.9.18-knowledge-tenant-isolation`)
- Current business-feature source / 当前业务功能源码：implementation range `dbbceb3` through `4df9104`; schema/runtime implementation is in `e883435`, `1758371`, and `14dd605`, followed by exact release-gate fixes through `4df9104` / 业务实现范围为 `dbbceb3` 至 `4df9104`，schema/运行时核心提交为 `e883435`、`1758371`、`14dd605`，其后发布门禁精确修复至 `4df9104`
- Current release-controller source / 当前发布控制器源码：`4df9104c0c18911a66a51277fd0a774aabe7ff35` (schema-v24 trusted release inventory and historical-v23 verifier compatibility / schema v24 可信发布清单与历史 v23 验证兼容)
...
- Release / 版本：`v0.9.18-knowledge-tenant-isolation`, deployed and verified on `2026-09-19` / 已于 `2026-09-19` 部署并验收。
- Production run / 生产运行：`4ea0c0d5c1d146568e28cd9f8a4b3591`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260919-012935`；candidate SHA-256 / 候选摘要：`8e20165a6b97f3d586bc2ee2fcfbf55c8fc3af9828b5953657a41f22de5dd934`。
- Acceptance-time runtime / 验收时运行状态：PM2 `online`, restart count `0`; Nginx is valid; public `/api/health` is `200` with parser ready; schema v24, SQLite integrity `ok`, foreign-key violations `0`, remote candidate tests `76/76`, and deployment Chromium smoke `2/2` passed / PM2 在线且无重启，Nginx、公网健康、解析器、schema v24、SQLite 完整性、远端候选测试和部署浏览器冒烟均正常。
...
- Database / 数据库：schema `v24`, integrity `ok`, foreign-key violations `0`; all `128` knowledge entries belong to organization `1`, null owners are `0`, organization count is `1`, and five organization-aware indexes plus six fail-closed ownership/custody triggers are present / schema v24、完整性与外键正常；128 条知识全部归属组织 1，空归属为 0，组织数为 1，五个组织范围索引和六个失败关闭的归属/保管触发器齐全。
- Phase 8 current slices / 第 8 阶段当前切片：the fail-closed module/action foundation, authoritative company-owner/read-only facts, scoped member governance, live read-only barrier, four CRM permission families, customer task workspace, task reassignment/editing, atomic organization-owner transfer, protected-account reset, campaign-performance export, customer-report export, influencer-data export/import authorizations, influencer row-level tenant isolation, and knowledge tenant isolation are active. Remaining modules stay separate. / 默认拒绝权限底座、企业所有者/只读权威事实、成员治理、CRM 权限与任务工作区、所有权转移、受保护账号重置、三类导出/导入权限、网红及知识库租户隔离均已上线；其余模块继续分离。
- Online authorization / 线上权限：ordinary knowledge ingest/search/categories/similar/usage, AI/RAG, proposal/PPT context, and business-artifact producers require the active authenticated organization. Platform administrators retain global knowledge and AI-conversation audit only in explicit Admin views; AI generation remains organization-scoped. / 普通知识入库/检索/分类/相似查询/使用计数、AI/RAG、方案/PPT 上下文及业务产物均强制活动认证组织；平台管理员仅在显式管理审计界面保留全局知识与 AI 对话可见性，AI 生成仍按组织隔离。
...
- v0.9.18 deployment note / v0.9.18 发布说明：historical v23 SQL/index framing and stale v23 release assertions stopped rejected candidates before production mutation. Exact regressions and independent re-review closed both issues; one interrupted pre-mutation candidate was safely recovered. Run `4ea0c0d5c1d146568e28cd9f8a4b3591` then completed with `DEPLOY_OK` and `RETENTION_CLEANUP_OK`. / 历史 v23 SQL/索引结构及旧 v23 发布断言均在生产变更前阻断候选；精确回归和独立复审关闭问题，一次变更前中断也已安全恢复，正式运行最终成功完成。
...
- Full evidence / 完整证据：`docs/version-records/2026-09-19-v0.9.18-knowledge-tenant-isolation-production.md`。
...
Production runs accepted v0.9.18 on schema v24. It preserves the accepted product shell, all v0.9.17 capabilities, and frozen PPT renderer while enforcing active-organization ownership across ordinary knowledge, AI/RAG, proposal/PPT context, and business-artifact paths; explicit Admin audit views remain global. Subsequent ordinary slices use affected tests plus one independent review, verified backup, immediate deployment, and online feature smoke; heavy full-suite testing remains phase-closeout or risk-triggered. / 生产现运行已验收的 v0.9.18/schema v24，保留既有产品壳层、v0.9.17 全部能力与冻结 PPT，并在普通知识、AI/RAG、方案/PPT 上下文及业务产物路径执行活动组织归属；显式管理审计仍保持全局。后续普通切片继续采用受影响测试、一次独立审查、可验证备份、当轮部署和线上功能冒烟。
