...
- Current accepted production source / 当前已验收生产源码：`1ffebb215d46cae60b99b58d82045897a002d4ea` (`v0.9.16-influencer-data-import-permission`)
- Current business-feature source / 当前业务功能源码：`971cc4ade6a9f3afb6698e2e617749c2edd994f4` (`v0.9.16-influencer-data-import-permission`); deployment inventory fix `1ffebb215d46cae60b99b58d82045897a002d4ea` includes the new service in the pinned candidate without replacing the accepted product shell / 业务功能提交为 `971cc4a`，部署清单修复 `1ffebb2` 将新服务纳入固定候选，不替换已验收产品壳层
- Current release-controller source / 当前发布控制器源码：`512e50abce569bbf6b0540b606a8e79ddeaa4ae2` (`fix(deploy): keep pinned transport pipeline silent`)
...
- Release / 版本：`v0.9.16-influencer-data-import-permission`, deployed and verified on `2026-09-18` / 已于 `2026-09-18` 部署并验收。
- Production run / 生产运行：`a86fc82caead46eaadbd19fe875e1b51`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260918-185009`；candidate SHA-256 / 候选摘要：`294de366e16567f8d4b984bccfaf05e00819b434fe96e517445b8410a8935a7f`。
- Acceptance-time runtime / 验收时运行状态：PM2 `online`, restart count `0`; Nginx and `pm2-root` `active`; public `/api/health` is `200` with parser ready; schema v22, SQLite `quick_check=ok`, foreign-key violations `0`, active sessions `0`, remote candidate tests `76/76`, and deployment Chromium smoke `2/2` passed / PM2 在线且无重启，Nginx、pm2-root、公网健康、解析器、schema v22、SQLite 完整性、远端候选测试和部署浏览器冒烟均正常，活动会话为零。
...
- Phase 8 current slices / 第 8 阶段当前切片：the fail-closed module/action foundation, authoritative company-owner/read-only facts, scoped member governance, live read-only barrier, four CRM permission families, customer task workspace, task reassignment/editing, atomic organization-owner transfer, protected-account reset, campaign-performance export, customer-report export, and influencer-data export/import authorizations are active. Remaining modules stay separate. / 默认拒绝权限底座、企业所有者/只读权威事实、成员治理、实时只读拦截、CRM 四类权限、客户任务工作区、任务改派/编辑、原子化组织所有权转移、受保护账号重置、Campaign 效果导出、客户报告导出及网红数据导出/导入权限均已上线；其余模块继续分离。
- Online authorization / 线上权限：`influencer.data.import` protects manual create, JSON import, and multipart preview/import paths and is projected through login plus `/api/auth/me`; `influencer.data.export` remains active. Production acceptance verified one-row multipart preview, bounded preview audit, zero business writes, eight enabled administrator import controls, logout, and token revocation. The existing influencer inventory is global and has no `org_id`; this release gates the actor action and does not claim row-level tenant isolation. / 网红手工新增、JSON 导入及上传预览/导入强制执行 `influencer.data.import`，既有导出权限继续有效；生产验收覆盖一行上传预览、有界审计、零业务写入、八个管理员导入控件、注销及令牌失效。现有网红主表无 `org_id`，本版仅限制操作者动作，不宣称行级租户隔离。
...
- v0.9.16 deployment note / v0.9.16 发布说明：the first candidate stopped before production mutation because the new permission service was absent from the pinned deployment inventory; recovery cleaned only the candidate and restored the control plane. After adding the exact inventory contract, the accepted run completed normally with `DEPLOY_OK` and `RETENTION_CLEANUP_OK`. / 首个候选因新权限服务未进入固定部署清单而在生产变更前停止，仅清理候选并恢复控制面；补齐清单与合同测试后，正式运行正常完成部署与保留清理。
...
- Full evidence / 完整证据：`docs/version-records/2026-09-18-v0.9.16-influencer-data-import-permission-production.md`。
...
Production runs accepted v0.9.16 on schema v22. The release preserves the accepted v0.6 product shell, all v0.9.15 capabilities, and frozen PPT renderer while adding the bounded influencer-data import actor permission. The global influencer table still requires a separate `org_id` schema release for row-level tenant isolation. Subsequent ordinary slices use affected tests plus one independent review, verified backup, immediate deployment, and online feature smoke; heavy full-suite testing is reserved for phase closeout or authentication, authorization architecture, schema, shared infrastructure, external-write, and other broad-risk changes. / 生产现运行已验收的 v0.9.16/schema v22，保留既有产品壳层、v0.9.15 全部能力与冻结 PPT，并增加有界网红数据导入操作者权限；行级租户隔离仍需独立 `org_id` schema 版本。后续普通切片继续采用受影响测试、一次独立审查、可验证备份、当轮部署和线上功能冒烟。
