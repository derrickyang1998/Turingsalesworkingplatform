...
- Current production feature source / 当前生产功能源码：`8d074ea17ca4f6d732e702c464f651ff82abc71f` (`v0.9.12-campaign-performance-export-permission`)
- Current release-controller source / 当前发布控制器源码：`84ae78f45604982482124d3dbd45b119f90acf48` (`fix(deploy): recover inactive accepted-release gateway`); this post-deploy controller fix is pushed for subsequent releases and is not represented as a second full production runtime deployment / 该部署后控制器修复已推送供后续发布使用，不记作第二次整包生产运行时部署
...
- Release / 版本：`v0.9.12-campaign-performance-export-permission`, deployed, recovered, and verified on `2026-09-18` / 已于 `2026-09-18` 部署、恢复并验收。
- Production run / 生产运行：`2f230c3af0ec461e9b04a7ab3723664c`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260918-103933`；candidate SHA-256 / 候选摘要：`567dbdd427f0e9c8136892544811b4e9fdc751d34ac6fcb543c6ee1f4c588fd2`。
- Acceptance-time runtime / 验收时运行状态：PM2 `online`, restart count `0`; Nginx and `pm2-root` `active`; public `/api/health` is `200` with parser ready; schema v22, SQLite integrity, export authorization, bounded denial audits, and focused desktop/mobile Chromium journeys passed / PM2 在线且无重启，Nginx、pm2-root、公网健康、解析器、schema v22、SQLite 完整性、导出权限、有界拒绝审计及本功能桌面/手机浏览器验收正常。
...
- Database / 数据库：schema `v22`, `quick_check=ok`, foreign-key violations `0`, governance tables/triggers present, assigned owner count `1`, owner authority version `3`, active sessions `0` / schema v22、完整性与外键正常、治理表和触发器存在、企业所有者唯一、权威版本为 3、活动会话为零。
- Phase 8 current slices / 第 8 阶段当前切片：the fail-closed module/action foundation, authoritative company-owner/read-only facts, scoped member governance, live read-only barrier, four CRM permission families, customer task workspace, task reassignment/editing, atomic organization-owner transfer, protected-account reset, and campaign-performance export authorization are active. Remaining modules stay separate. / 默认拒绝权限底座、企业所有者/只读权威事实、成员治理、实时只读拦截、CRM 四类权限、客户任务工作区、任务改派/编辑、原子化组织所有权转移、受保护账号重置及 Campaign 效果导出权限均已上线；其余模块继续分离。
- Online authorization / 线上权限：`campaign.performance.export` is enforced before CSV generation on both content and Feishu snapshot exports; read-only requests return bounded JSON `403` evidence with no attachment bytes, while authorized content export returns CSV. Focused desktop/mobile UI acceptance passed and active sessions are `0`. / 两个效果导出端点均在生成 CSV 前强制执行 `campaign.performance.export`；只读请求返回有界 JSON 403 且无附件字节，授权内容导出返回 CSV；桌面/手机定向验收通过，活动会话为 0。
...
- Recovery note / 恢复说明：the accepted run required fail-closed finalization recovery after inactive Nginx could not be reloaded. The corrected controller starts only a validated closed maintenance listener before arming; focused recovery `3/3`, affected infrastructure `119` pass / `0` fail / `1` platform skip, and independent re-review `APPROVE`. / 已接纳运行因停止状态 Nginx 无法 reload 而执行失败关闭最终化恢复；修正控制器仅在 arm 前启动经过验证的 closed 维护监听，聚焦恢复与受影响基础设施门禁及独立复审均通过。
...
- Full evidence / 完整证据：`docs/version-records/2026-09-18-v0.9.12-campaign-performance-export-permission-production.md`。
...
The candidate lives under `/var/lib/turingmarket-gate/releases`. The accepted v0.9.11 gate supports exact schema-v22 no-op adoption and constrains the legitimate CRM `task_updated` audit value to the v22 structural policy; historical schemas and unknown v22 values remain fail closed. Migration code never runs as root or receives the production database. Per-feature validation stays bounded to migration/replay, route/static, Nginx, and the named feature journey; full Node and browser matrices remain phase-closeout or risk-trigger checks. / 候选版本位于受限发布目录；v0.9.11 已验收门禁支持精确 schema v22 无操作接纳，并将合法 CRM `task_updated` 审计值严格限定在 v22 结构策略中，历史 schema 与未知 v22 值继续失败关闭。迁移代码不以 root 运行且不取得生产数据库；单功能门禁继续限定于迁移/重放、路由/静态资源、Nginx 及命名功能旅程，完整 Node 与浏览器矩阵保留到阶段收口或风险触发。
...
Production runs accepted v0.9.11 on schema v22. The release preserves the accepted v0.6 product shell and frozen PPT renderer while retaining exact CRM permissions, task editing, organization-owner transfer, and protected-account recovery boundaries. Subsequent ordinary slices use affected tests plus one independent review, verified backup, immediate deployment, and online feature smoke; authentication, authorization architecture, schema, shared infrastructure, and external-write changes retain risk-triggered gates. / 生产现运行已验收的 v0.9.11/schema v22，保留既有产品壳层与冻结 PPT、精确 CRM 权限、任务编辑、组织所有权转移及受保护账号恢复边界；后续普通切片采用受影响测试、一次独立审查、可验证备份、当轮部署和线上功能冒烟，高风险边界继续保留必要门禁。
