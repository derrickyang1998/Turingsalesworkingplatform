...
- Current accepted production source / 当前已验收生产源码：`b25f53361e21acb5c32e2655249098837630cecb` (`v0.9.17-influencer-tenant-isolation`)
- Current business-feature source / 当前业务功能源码：`6e70032f3bca55d772c193f6fa43e837a2a42a58` (`v0.9.17-influencer-tenant-isolation`); migration compatibility fix `b25f53361e21acb5c32e2655249098837630cecb` admits only the strict production-sanitizer default-organization fingerprint / 业务功能提交为 `6e70032`，迁移兼容修复 `b25f533` 仅接纳严格的生产脱敏默认组织指纹
...
- Release / 版本：`v0.9.17-influencer-tenant-isolation`, deployed and verified on `2026-09-18` / 已于 `2026-09-18` 部署并验收。
- Production run / 生产运行：`7f0d59666fc1434491d4655a4aff5f56`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260918-221444`；candidate SHA-256 / 候选摘要：`7d031b274bee712ab22de1c8e93fe7b316b6838cf503dbe55c87d82e7bbf17c7`。
- Acceptance-time runtime / 验收时运行状态：PM2 `online`, restart count `0`; Nginx is valid; public `/api/health` is `200` with parser ready; schema v23, SQLite integrity `ok`, foreign-key violations `0`, remote candidate tests `76/76`, and deployment Chromium smoke `2/2` passed / PM2 在线且无重启，Nginx、公网健康、解析器、schema v23、SQLite 完整性、远端候选测试和部署浏览器冒烟均正常。
...
- Database / 数据库：schema `v23`, integrity `ok`, foreign-key violations `0`; all `4,786` influencers belong to organization `1`, null/unknown owners are `0`, and the three ownership indexes plus two fail-closed triggers are present / schema v23、完整性与外键正常；4,786 条网红全部归属组织 1，空/未知归属为 0，三个索引和两个失败关闭触发器齐全。
- Phase 8 current slices / 第 8 阶段当前切片：the fail-closed module/action foundation, authoritative company-owner/read-only facts, scoped member governance, live read-only barrier, four CRM permission families, customer task workspace, task reassignment/editing, atomic organization-owner transfer, protected-account reset, campaign-performance export, customer-report export, influencer-data export/import authorizations, and influencer row-level tenant isolation are active. Remaining modules stay separate. / 默认拒绝权限底座、企业所有者/只读权威事实、成员治理、CRM 权限与任务工作区、所有权转移、受保护账号重置、三类导出/导入权限及网红行级租户隔离均已上线；其余模块继续分离。
- Online authorization / 线上权限：actor permissions still protect import/export, while every influencer read/write, Campaign shortlist/workspace, linked order, publication evidence, and Feishu selection now requires matching authenticated organization ownership. Deployed-code cross-organization smoke passed `2/2` without production business-row mutation. / 导入/导出操作者权限继续生效，网红读写、Campaign 候选/工作台、关联订单、发布凭证及飞书选择现同时强制认证组织归属；线上已部署代码跨组织冒烟 `2/2`，未修改生产业务数据。
...
- v0.9.17 deployment note / v0.9.17 发布说明：the first candidate stopped before production mutation because migration 023 did not recognize the sanitizer-rewritten default organization. Recovery restored the control plane without changing production. After the bounded compatibility fix, exact tests, and independent approval, run `7f0d59666fc1434491d4655a4aff5f56` completed with `DEPLOY_OK` and `RETENTION_CLEANUP_OK`. / 首个候选因迁移 023 无法识别脱敏默认组织而在生产变更前停止；限定兼容修复、精确测试及独立批准后正式运行成功完成。
...
- Full evidence / 完整证据：`docs/version-records/2026-09-18-v0.9.17-influencer-tenant-isolation-production.md`。
...
Production runs accepted v0.9.17 on schema v23. It preserves the accepted v0.6 product shell, all v0.9.16 capabilities, and frozen PPT renderer while enforcing organization ownership across influencer, Campaign, collaboration, publication, and Feishu-selection paths. Subsequent ordinary slices use affected tests plus one independent review, verified backup, immediate deployment, and online feature smoke; heavy full-suite testing remains phase-closeout or risk-triggered. / 生产现运行已验收的 v0.9.17/schema v23，保留既有产品壳层、v0.9.16 全部能力与冻结 PPT，并在网红、Campaign、合作、发布及飞书选择路径执行组织归属。后续普通切片继续采用受影响测试、一次独立审查、可验证备份、当轮部署和线上功能冒烟。
