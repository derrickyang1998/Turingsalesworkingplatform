...
- Current accepted production source / 当前已验收生产源码：`b9874d59fa5742aaf36569e4f9f328848b401673` (`v0.9.23-plan-catalog-module-entitlements`)
- Current business-feature source / 当前业务功能源码：implementation range `dbbceb3` through `b9874d5`; token-usage schema/runtime ownership is in `b4d6ba0`, unified server-side AI quota admission/accounting is in `f1f9c98`, demand XLSX parser compatibility is in `df83db7` plus `8f3f070`, and the plan-catalog/module-entitlement slice is in `1509d5b` through `b9874d5` / 业务实现范围为 `dbbceb3` 至 `b9874d5`；Token 用量租户归属位于 `b4d6ba0`，服务端 AI 配额准入位于 `f1f9c98`，需求 XLSX 兼容位于 `df83db7` 与 `8f3f070`，套餐目录及模块权益位于 `1509d5b` 至 `b9874d5`
- Current release-controller source / 当前发布控制器源码：`b9874d5` (schema-v27 trusted inventory, plan-entitlement candidate gate, normalized source bytes, and verified interruption-recoverable retention / schema v27 可信清单、套餐权益候选门禁、源码字节规范化及可验证可中断恢复的保留控制)
...
- Release / 版本：`v0.9.23-plan-catalog-module-entitlements`, deployed and verified on `2026-09-20` / 已于 `2026-09-20` 部署并验收。
- Production run / 生产运行：`53b27349f77045ceb5f0a2773339a7c8`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260920-210744`；candidate SHA-256 / 候选摘要：`d4b46ef30be71becb967b7d8c2c932cd0f79db59c7d596aa569a5c2c65bb5aa9`。
- Acceptance-time runtime / 验收时运行状态：PM2 `online`, restart count `0`; Nginx is valid; public `/api/health` is `200` with parser ready; schema v27, SQLite `quick_check=ok`, foreign-key violations `0`, focused entitlement tests `76/76`, full migration replay `8/8`, and deployment Chromium smoke `2/2` passed / PM2 在线且无重启，Nginx、公网健康、解析器、schema v27、SQLite 完整性、套餐权益定向测试、完整迁移重放和部署浏览器冒烟均正常。
...
- Database / 数据库：schema `v27`, integrity `ok`, foreign-key violations `0`; the immutable plan/module catalog and append-only contiguous organization assignment history are active, while the existing token-usage ledger protections remain intact / schema v27、完整性与外键正常；不可变套餐/模块目录及连续仅追加的组织分配历史已启用，既有 Token 用量账本保护保持不变。
- Phase 8 current slices / 第 8 阶段当前切片：the fail-closed module/action foundation, authoritative roles, scoped member governance, CRM and export/import action policies, influencer/knowledge/AI-conversation/token-usage tenant ownership, server-side AI token quota admission, immutable plan catalog, and server-enforced module entitlements are active. Subscription expiry, organization monthly shared quota, billing, and persistent concurrency reservation remain separate releases. / 默认拒绝权限底座、权威角色、成员治理、CRM 与导出/导入动作权限、四类租户归属、服务端 AI Token 配额准入、不可变套餐目录及服务端模块权益均已上线；订阅到期、组织月度共享配额、账单和持久并发预留继续拆分交付。
...
- Full evidence / 完整证据：`docs/version-records/2026-09-20-v0.9.23-plan-catalog-module-entitlements-production.md`。
...
Production runs accepted v0.9.23 on schema v27. It preserves the accepted product shell and all earlier tenant/quota controls while adding an immutable plan/module catalog, append-only organization plan history, and server-side role-permission intersection with live plan entitlements. Subsequent ordinary slices use affected tests plus one independent review, verified backup, immediate deployment, and online feature smoke; heavy full-suite testing remains phase-closeout or risk-triggered. / 生产现运行已验收的 v0.9.23/schema v27，保留产品壳层及此前全部租户/配额控制，并新增不可变套餐/模块目录、仅追加组织套餐历史及服务端实时角色权限与套餐权益交集。后续普通切片继续采用受影响测试、一次独立审查、可验证备份、当轮部署和线上功能冒烟；重型全量测试保留到阶段收口或风险触发。
