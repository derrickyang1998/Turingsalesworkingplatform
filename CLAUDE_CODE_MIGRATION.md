...
- Current accepted production source / 当前已验收生产源码：`512e50abce569bbf6b0540b606a8e79ddeaa4ae2` (`v0.9.13-pinned-candidate-bundle-upload`)
- Current business-feature source / 当前业务功能源码：`8d074ea17ca4f6d732e702c464f651ff82abc71f` (`v0.9.12-campaign-performance-export-permission`); v0.9.13 changes only the shared release transport and preserves this product surface / v0.9.13 只改共享发布传输，业务界面与能力仍保持 v0.9.12
...
- Release / 版本：`v0.9.13-pinned-candidate-bundle-upload`, deployed and verified on `2026-09-18` / 已于 `2026-09-18` 部署并验收。
- Production run / 生产运行：`d2b3a03d53274f67b086b9df3a341f90`; backup / 备份：`/root/turingmarket/backups/v060-crm-sales-workspace-20260918-132223`；candidate SHA-256 / 候选摘要：`807521076e9c65088a570c48def78f3e70ea33d43c8dbe63c150dcc497323236`。
...
- Full evidence / 完整证据：`docs/version-records/2026-09-18-v0.9.13-pinned-candidate-bundle-upload-production.md`。
...
Production runs accepted v0.9.13 on schema v22. The release preserves the accepted v0.6 product shell, the v0.9.12 business surface, and frozen PPT renderer while using the single-stream pinned upload controller. Subsequent ordinary slices use affected tests plus one independent review, verified backup, immediate deployment, and online feature smoke; authentication, authorization architecture, schema, shared infrastructure, and external-write changes retain risk-triggered gates. / 生产现运行已验收的 v0.9.13/schema v22，保留既有产品壳层、v0.9.12 业务能力与冻结 PPT，并使用单连接钉住上传控制器；后续普通切片采用受影响测试、一次独立审查、可验证备份、当轮部署和线上功能冒烟，高风险边界继续保留必要门禁。
