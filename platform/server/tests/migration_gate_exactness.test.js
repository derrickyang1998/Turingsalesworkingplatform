...
test('migration verifier accepts a populated sanitized version 1 source and reaches version 20', (t) => {
  const fixture = createFixture(t, 'v1-to-v20', 1);
...
test('migration verifier accepts a populated managed version 9 source and upgrades it to version 20', (t) => {
  const fixture = createFixture(t, 'v9-to-v20', 9);
  const sourceSha256 = sha256File(fixture.databasePath);
...
test('migration verifier upgrades an existing managed version 10 source to version 20', (t) => {
  const fixture = createFixture(t, 'v10-to-v20', 10);
  const sourceSha256 = sha256File(fixture.databasePath);
...
test('migration verifier upgrades an existing managed version 11 source to version 20', (t) => {
  const fixture = createFixture(t, 'v11-to-v20', 11);
  const sourceSha256 = sha256File(fixture.databasePath);
...
test('migration verifier upgrades the production predecessor version 19 to version 20', (t) => {
  const fixture = createFixture(t, 'v19-to-v20', 19);
  const sourceSha256 = sha256File(fixture.databasePath);
  ...
  assert.equal(report.sourceVersion, 19);
  assert.equal(report.targetVersion, 20);
  assert.equal(report.runs, 2);
  assert.equal(report.preMigrationRestoreVerified, true);
  assert.equal(report.legacyPreservationVerified, true);
