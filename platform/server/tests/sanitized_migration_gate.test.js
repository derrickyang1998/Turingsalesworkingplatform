...
test('schema v31 sanitizer rebuilds organization billing statement digests', () => {
  const db = new Database(':memory:');
  try {
    db.exec(`
      CREATE TABLE organization_billing_policies (
        org_id INTEGER NOT NULL,policy_version INTEGER NOT NULL,effective_month TEXT NOT NULL,
        billing_enabled INTEGER NOT NULL,currency TEXT NOT NULL,base_fee_cents INTEGER NOT NULL,
        included_tokens INTEGER NOT NULL,overage_cents_per_million_tokens INTEGER NOT NULL,
        PRIMARY KEY (org_id,policy_version)
      ) STRICT;
      CREATE TABLE organization_billing_statements (
        id INTEGER PRIMARY KEY,org_id INTEGER NOT NULL,period_key TEXT NOT NULL,
        period_start TEXT NOT NULL,period_end TEXT NOT NULL,policy_version INTEGER NOT NULL,
        billing_enabled INTEGER NOT NULL,currency TEXT NOT NULL,usage_tokens INTEGER NOT NULL,
        usage_record_count INTEGER NOT NULL,usage_max_id INTEGER,included_tokens INTEGER NOT NULL,
        billable_tokens INTEGER NOT NULL,base_fee_cents INTEGER NOT NULL,
        overage_cents_per_million_tokens INTEGER NOT NULL,overage_fee_cents INTEGER NOT NULL,
        total_cents INTEGER NOT NULL,formula_version TEXT NOT NULL,statement_sha256 TEXT NOT NULL,
        closed_by INTEGER NOT NULL,reason TEXT NOT NULL,created_at TEXT NOT NULL
      ) STRICT;
      INSERT INTO organization_billing_policies VALUES
        (10,2,'2026-08-01',1,'USD',1000,1000000,250);
      INSERT INTO organization_billing_statements VALUES (
        1,10,'2026-08','2026-08-01 00:00:00','2026-09-01 00:00:00',2,1,'USD',
        1500000,2,42,1000000,500000,1000,250,125,1125,'tm-billing-v1',
        '${'0'.repeat(64)}',3,'monthly close','2026-09-21 08:00:00'
      );
    `);
    sanitizer._testing.rebuildOrganizationBillingStatementDigests(db);
    assert.equal(
      db.prepare('SELECT statement_sha256 FROM organization_billing_statements WHERE id=1').get().statement_sha256,
...
test('manifest declares exact managed v1 as primary and keeps isolated v6 through v31 profiles', () => {
  ...
  const v22Fixture = migratedFixture('manifest-v22-isolated', 22);
  const v23Fixture = migratedFixture('manifest-v23-isolated', 23);
  const v24Fixture = migratedFixture('manifest-v24-isolated', 24);
  const v25Fixture = migratedFixture('manifest-v25-isolated', 25);
  const v26Fixture = migratedFixture('manifest-v26-isolated', 26);
  const v27Fixture = migratedFixture('manifest-v27-isolated', 27);
  const v28Fixture = migratedFixture('manifest-v28-isolated', 28);
  const v29Fixture = migratedFixture('manifest-v29-isolated', 29);
  const v30Fixture = migratedFixture('manifest-v30-isolated', 30);
  ...
  try {
    ...
    const v22Profile = sanitizer._testing.manifestProfileForVersion(manifest, 22);
    const v23Profile = sanitizer._testing.manifestProfileForVersion(manifest, 23);
    const v24Profile = sanitizer._testing.manifestProfileForVersion(manifest, 24);
    const v25Profile = sanitizer._testing.manifestProfileForVersion(manifest, 25);
    const v26Profile = sanitizer._testing.manifestProfileForVersion(manifest, 26);
    const v27Profile = sanitizer._testing.manifestProfileForVersion(manifest, 27);
    const v28Profile = sanitizer._testing.manifestProfileForVersion(manifest, 28);
    const v29Profile = sanitizer._testing.manifestProfileForVersion(manifest, 29);
    const v30Profile = sanitizer._testing.manifestProfileForVersion(manifest, 30);
    const v31Profile = sanitizer._testing.manifestProfileForVersion(manifest, 31);
    ...
    assert.equal(v22Profile.schemaVersion, 22);
    assert.equal(v23Profile.schemaVersion, 23);
    assert.equal(v24Profile.schemaVersion, 24);
    assert.equal(v25Profile.schemaVersion, 25);
    assert.equal(v26Profile.schemaVersion, 26);
    assert.equal(v27Profile.schemaVersion, 27);
    assert.equal(v28Profile.schemaVersion, 28);
    assert.equal(v29Profile.schemaVersion, 29);
    assert.equal(v30Profile.schemaVersion, 30);
    assert.equal(v31Profile.schemaVersion, 31);
    ...
    assert.equal(v22Profile.objects.length, sanitizer.actualInventory(v22Fixture.db).length);
    assert.equal(v23Profile.objects.length, sanitizer.actualInventory(v23Fixture.db).length);
    assert.equal(v24Profile.objects.length, sanitizer.actualInventory(v24Fixture.db).length);
    assert.equal(v25Profile.objects.length, sanitizer.actualInventory(v25Fixture.db).length);
    assert.equal(v26Profile.objects.length, sanitizer.actualInventory(v26Fixture.db).length);
    assert.equal(v27Profile.objects.length, sanitizer.actualInventory(v27Fixture.db).length);
    assert.equal(v28Profile.objects.length, sanitizer.actualInventory(v28Fixture.db).length);
    assert.equal(v29Profile.objects.length, sanitizer.actualInventory(v29Fixture.db).length);
    assert.equal(v30Profile.objects.length, sanitizer.actualInventory(v30Fixture.db).length);
    assert.equal(v31Profile.objects.length, sanitizer.actualInventory(v31Fixture.db).length);
    ...
    const v22Authority = v22Profile.objects.find((object) => object.name === 'organization_authority');
    assert.ok(v22Authority);
    assert.ok(v22Authority.columns.some((column) => column.name === 'updated_by'));
    assert.ok(v22Authority.columns.some((column) => column.name === 'updated_at'));
    assert.ok(v22Authority.columns.some((column) => column.name === 'version'));
    const v23Influencers = v23Profile.objects.find((object) => object.name === 'influencers');
    assert.ok(v23Influencers);
    assert.deepEqual(
      v23Influencers.columns.find((column) => column.name === 'org_id'),
      {
        name: 'org_id',
    ...
    const v24KnowledgeEntries = v24Profile.objects.find((object) => object.name === 'knowledge_entries');
    assert.ok(v24KnowledgeEntries);
    assert.deepEqual(
      v24KnowledgeEntries.columns.find((column) => column.name === 'org_id'),
      {
        name: 'org_id',
        declaredType: 'INTEGER',
        notnull: 0,
    ...
    const v25Conversations = v25Profile.objects.find((object) => object.name === 'ai_conversations');
    assert.ok(v25Conversations);
    assert.equal(v25Conversations.columns.find((column) => column.name === 'org_id').classification, 'structural');
    const v26TokenUsage = v26Profile.objects.find((object) => object.name === 'token_usage');
    assert.ok(v26TokenUsage);
    assert.equal(v26TokenUsage.columns.find((column) => column.name === 'org_id').classification, 'structural');
    const v27Plans = v27Profile.objects.find((object) => object.name === 'plan_catalog');
    const v27Assignments = v27Profile.objects.find((object) => object.name === 'organization_plan_assignments');
    assert.ok(v27Plans);
    assert.ok(v27Assignments);
    assert.equal(v27Plans.columns.find((column) => column.name === 'code').classification, 'structural');
    assert.equal(v27Assignments.columns.find((column) => column.name === 'org_id').classification, 'structural');
    assert.equal(v27Assignments.columns.find((column) => column.name === 'reason').classification, 'synthetic-text');
    const v28Terms = v28Profile.objects.find((object) => object.name === 'organization_subscription_terms');
    assert.ok(v28Terms);
    assert.equal(v28Terms.columns.find((column) => column.name === 'org_id').classification, 'structural');
    assert.equal(v28Terms.columns.find((column) => column.name === 'term_version').classification, 'structural');
    assert.equal(v28Terms.columns.find((column) => column.name === 'expires_at').classification, 'structural');
    assert.equal(v28Terms.columns.find((column) => column.name === 'reason').classification, 'synthetic-text');
    const v29QuotaPolicies = v29Profile.objects.find((object) => object.name === 'organization_ai_quota_policies');
    assert.ok(v29QuotaPolicies);
    assert.equal(v29QuotaPolicies.columns.find((column) => column.name === 'org_id').classification, 'structural');
    assert.equal(v29QuotaPolicies.columns.find((column) => column.name === 'policy_version').classification, 'structural');
    assert.equal(v29QuotaPolicies.columns.find((column) => column.name === 'monthly_limit').classification, 'structural');
    assert.equal(v29QuotaPolicies.columns.find((column) => column.name === 'reason').classification, 'synthetic-text');
    assert.equal(v29QuotaPolicies.columns.find((column) => column.name === 'source').classification, 'structural');
    const v30ConcurrencyPolicies = v30Profile.objects.find((object) => object.name === 'organization_ai_concurrency_policies');
    const v30Reservations = v30Profile.objects.find((object) => object.name === 'ai_provider_reservations');
    const v30ReservationEvents = v30Profile.objects.find((object) => object.name === 'ai_provider_reservation_events');
    assert.ok(v30ConcurrencyPolicies);
    assert.ok(v30Reservations);
    assert.ok(v30ReservationEvents);
    assert.equal(v30ConcurrencyPolicies.columns.find((column) => column.name === 'org_id').classification, 'structural');
    assert.equal(v30ConcurrencyPolicies.columns.find((column) => column.name === 'concurrency_limit').classification, 'structural');
    assert.equal(v30ConcurrencyPolicies.columns.find((column) => column.name === 'reason').classification, 'synthetic-text');
    assert.equal(v30Reservations.columns.find((column) => column.name === 'reservation_id').classification, 'structural');
    assert.equal(v30Reservations.columns.find((column) => column.name === 'operation_key').classification, 'synthetic-text');
    assert.equal(v30Reservations.columns.find((column) => column.name === 'fence_token').classification, 'secret-synthetic');
    assert.equal(v30Reservations.columns.find((column) => column.name === 'state').classification, 'structural');
    assert.equal(v30ReservationEvents.columns.find((column) => column.name === 'reservation_id').classification, 'structural');
    assert.equal(v30ReservationEvents.columns.find((column) => column.name === 'metadata_json').classification, 'json-leaves');
    const v31Policies = v31Profile.objects.find((object) => object.name === 'organization_billing_policies');
    const v31Statements = v31Profile.objects.find((object) => object.name === 'organization_billing_statements');
    assert.ok(v31Policies);
    assert.ok(v31Statements);
    assert.equal(v31Policies.columns.find((column) => column.name === 'reason').classification, 'synthetic-text');
    assert.equal(v31Statements.columns.find((column) => column.name === 'usage_record_count').classification, 'structural');
    assert.equal(v31Statements.columns.find((column) => column.name === 'usage_max_id').classification, 'structural');
    assert.equal(v31Statements.columns.find((column) => column.name === 'statement_sha256').classification, 'dependent-digest');
    ...
    assert.equal(v23Profile.semanticPolicies.structuralColumns.validatorVersion, 'tm-structural-policy-v21-influencer-tenant-ownership');
    assert.equal(v24Profile.semanticPolicies.structuralColumns.validatorVersion, 'tm-structural-policy-v22-knowledge-tenant-ownership');
    assert.equal(v25Profile.semanticPolicies.structuralColumns.validatorVersion, 'tm-structural-policy-v23-ai-conversation-tenant-ownership');
    assert.equal(v26Profile.semanticPolicies.structuralColumns.validatorVersion, 'tm-structural-policy-v24-token-usage-tenant-ownership');
    assert.equal(v27Profile.semanticPolicies.structuralColumns.validatorVersion, 'tm-structural-policy-v25-plan-catalog-module-entitlements');
    assert.equal(v28Profile.semanticPolicies.structuralColumns.validatorVersion, 'tm-structural-policy-v26-subscription-expiry');
    assert.equal(v29Profile.semanticPolicies.structuralColumns.validatorVersion, 'tm-structural-policy-v27-organization-monthly-ai-quota');
    assert.equal(v30Profile.semanticPolicies.structuralColumns.validatorVersion, 'tm-structural-policy-v28-ai-provider-concurrency-reservation');
    assert.equal(v31Profile.semanticPolicies.structuralColumns.validatorVersion, 'tm-structural-policy-v29-organization-billing-statements');
    ...
    assert.doesNotThrow(() => sanitizer.validateManifest(manifest, v22Fixture.db));
    assert.doesNotThrow(() => sanitizer.validateManifest(manifest, v23Fixture.db));
    assert.doesNotThrow(() => sanitizer.validateManifest(manifest, v24Fixture.db));
    assert.doesNotThrow(() => sanitizer.validateManifest(manifest, v25Fixture.db));
    assert.doesNotThrow(() => sanitizer.validateManifest(manifest, v26Fixture.db));
    assert.doesNotThrow(() => sanitizer.validateManifest(manifest, v27Fixture.db));
    assert.doesNotThrow(() => sanitizer.validateManifest(manifest, v28Fixture.db));
    assert.doesNotThrow(() => sanitizer.validateManifest(manifest, v29Fixture.db));
    assert.doesNotThrow(() => sanitizer.validateManifest(manifest, v30Fixture.db));
    assert.doesNotThrow(() => sanitizer.validateManifest(manifest, v31Fixture.db));
  } finally {
    ...
    closeAndRemove(v22Fixture);
    closeAndRemove(v23Fixture);
    closeAndRemove(v24Fixture);
    closeAndRemove(v25Fixture);
    closeAndRemove(v26Fixture);
    closeAndRemove(v27Fixture);
    closeAndRemove(v28Fixture);
    closeAndRemove(v29Fixture);
    closeAndRemove(v30Fixture);
    closeAndRemove(v31Fixture);
  }
});
...
test('v16 sanitizer preserves inert PDF framing, filename constraints, byte length, and rebuilt digest', () => {
  ...
  try {
    ...
    const documentId = 881032;
    const knowledgeEntryId = 881033;
    const evidenceContent = '{"checkpoint":"contract_document"}';
    fixture.db.prepare(`
      INSERT INTO knowledge_entries (
        id,entry_type,source_type,source_id,key_terms,content,created_by,is_public,
    ...
    `).run({
      ...
      businessId: String(populated.campaignId),
      sourceIdentitySha256: sha256Text('sanitizer-contract-document-source-identity'),
      contentSha256: sha256Text(evidenceContent)
    });
    fixture.db.prepare(`
      INSERT INTO campaign_record_links (
        org_id,campaign_id,record_type,bundle_id,record_id,relation_type,created_by,metadata_json
    ...
    `).run(
      populated.orgId,
      populated.campaignId,
      sha256Text('sanitizer-contract-document-knowledge-link'),
