...
function seedAdminAndTeam(db) {
  const existingAdmin = db.prepare("SELECT id FROM users WHERE role = 'admin' LIMIT 1").get();
  if (existingAdmin) return { inserted: 0 };

  const defaultAdminUsername = configuredAdminUsername();
  const passwords = { admin: resolveSeedPassword('DEFAULT_ADMIN_PASSWORD', { requiredInProduction: true }), members: {} };
  TEAM_MEMBERS.forEach((member) => {
    passwords.members[member[0]] = resolveSeedPassword(userPasswordEnvKey(member[0]));
  });

  const insertUser = db.prepare('INSERT INTO users (username, password_hash, display_name, role, email, department, api_quota) VALUES (?, ?, ?, ?, ?, ?, ?)');
  insertUser.run(defaultAdminUsername, seedPasswordHash(passwords.admin), '管理员', 'admin', 'admin@turingmarket.cn', '管理', 200000);
  TEAM_MEMBERS.forEach(([username, displayName, role, email, dept]) => {
    try {
      insertUser.run(username, seedPasswordHash(passwords.members[username]), displayName, role, email, dept, 50000);
    } catch (error) {
      if (!/UNIQUE/i.test(String(error && error.message))) throw error;
    }
  });
  return { inserted: 1 + TEAM_MEMBERS.length };
}

function seedInfluencers(db) {
  const existingInfluencers = db.prepare('SELECT COUNT(*) AS count FROM influencers').get();
  if (existingInfluencers.count !== 0) return { inserted: 0 };
  ...
  for (const row of INFLUENCER_SEED_DATA) insertInf.run(...row);
  return { inserted: INFLUENCER_SEED_DATA.length };
}

const seedAdmissions = {
  admin: seedAdminAndTeam,
