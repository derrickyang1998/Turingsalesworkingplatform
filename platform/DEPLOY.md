...
The sealed runtime is fixed at 3,476 files, 435 directories, 640,592,293 bytes, with SHA-256 `04630b2ee928ba8031a4ea5ce6435519161c19e05884813e5ec623d12adb38ce`. It contains no symlinks or Python bytecode caches and is installed at `/var/lib/turingmarket-parser/runtime-root`. / 密封运行时的文件、目录、字节数与哈希如前，不包含软链接或 Python 字节码缓存，并安装到上述路径。
...
process.stdin.on("end", () => {
  const health = JSON.parse(body);
  const expected = "ddf02acd3832b38b465fb8fef16398ea00c0fbcf8cdc6011c8889626f6e8e021";
...
Check the installed appliance identity and spool without following links. Expected runtime identity is 3,476 files, 435 directories, 640,592,293 bytes, SHA-256 `04630b2ee928ba8031a4ea5ce6435519161c19e05884813e5ec623d12adb38ce`; the jobs directory must be empty outside active admissions. / 不跟随软链接检查已安装设备身份与 spool；预期运行时身份如前，非活动 admission 期间 jobs 目录必须为空。
