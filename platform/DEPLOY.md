...
- Guarded release branch / 受控发布分支：`codex/v0.7.0-ai-knowledge-proposal-ppt-loop-production`
...
- Current focused slice / 当前定向切片：Task 4D4 governance release hardening is production accepted. Trusted v1/v6/v7 migration admission, atomic deployment-smoke cleanup, Campaign governance eligibility, exact PM2 systemd startup, verified rollback, and online authorization/FTS checks pass; final independent review is `APPROVE` with no findings. Exact evidence is in the Task 4D4 version record. / Task 4D4 知识治理发布加固已通过生产验收；可信 v1/v6/v7 迁移准入、原子验收清理、Campaign 治理资格、精确 PM2 自启动、回滚及线上权限/FTS 验证均通过，最终独立复审无发现并 `APPROVE`，精确证据见 Task 4D4 版本记录。
...
- Production / 生产：accepted v0.6 shell plus v0.7 Phase 6 slices 1 through 4D4; App and PPT build markers remain unchanged / 已验收 v0.6 壳层叠加 v0.7 阶段 6 切片 1 至 4D4，App 与 PPT build 标识不变

### Incremental Delivery Cadence / 单功能增量发布节奏

Each completed feature slice runs only the affected unit/API/contract tests, syntax or migration checks, a focused secret scan, and one independent review. After a verified backup, it is deployed immediately and must pass production health, dedicated-smoke-identity or internal-session authentication, and core-path smoke checks. Protected owner/platform-administrator accounts are never release identities; a failed smoke login blocks the release and must never trigger credential rotation. The cutover verifies the snapshot security overlay after acceptance mutations and again before final public acceptance. Full non-browser regression, Playwright, and the complete reviewer matrix run at phase closeout, a scheduled release window, or whenever the change crosses authentication, authorization, migration, shared-infrastructure, or similarly broad risk boundaries. HIGH/CRITICAL findings, credential drift, backup/rollback readiness, migration safety, and production smoke remain hard blockers. / 每个功能完成后仅执行受影响的单元/API/合同测试、语法或迁移检查、聚焦敏感信息扫描和一次独立审查；完成可校验备份后立即上线，并使用专用冒烟身份或内部会话完成认证与核心路径检查。受保护的所有者/平台管理员账号不得作为发布身份；冒烟登录失败必须阻断发布且不得触发凭据轮换。切换流程会在业务验收变更后及公网最终验收前两次核对快照安全覆盖层。完整非浏览器回归、Playwright 与完整审查矩阵改在阶段收口、计划发布窗口或认证、权限、迁移、共享基础设施等跨模块高风险变更时执行。HIGH/CRITICAL、凭据漂移、备份/回滚、迁移安全和生产冒烟仍是硬阻断项。
...
The sealed runtime is fixed at 3,476 files, 435 directories, 640,588,717 bytes, with SHA-256 `24905b29e26fe68791a32e931d92322a2ee87df0ae08baded7dec70874455cba`. It contains no symlinks or Python bytecode caches and is installed at `/var/lib/turingmarket-parser/runtime-root`. / 密封运行时的文件、目录、字节数与哈希如前，不包含软链接或 Python 字节码缓存，并安装到上述路径。

Parser candidate preparation keeps two root-owned, content-addressed caches below `/var/lib/turingmarket-gate/parser-cache/v1`: a complete sealed runtime with its original build evidence, and the downloaded npm/Python dependency closure. A release first validates the runtime cache from canonical trusted inputs, the exact trusted runtime-builder digest, and a fresh full-tree measurement; only a miss proceeds to the dependency cache and the existing offline build unit. Every evidence file is canonical ASCII JSON, `root:root 0444`, single-link, and read without following symlinks. Invalid entries are atomically moved to the fixed quarantine area before rebuilding. A release interrupted by process kill or host restart also moves its release-named staging entry to quarantine, but only after proving the corresponding fetch unit is no longer active; unknown names, an active unit, unsafe control roots, or target ambiguity stop the next release without deletion. Cache reuse still creates the existing staged runtime and does not bypass installation, parser self-tests, production acceptance, or rollback. / 解析器候选准备在 `/var/lib/turingmarket-gate/parser-cache/v1` 下维护两层 root 所有、内容寻址缓存：带原始构建证据的完整密封运行时，以及已下载的 npm/Python 依赖闭包。发布会先根据规范化可信输入、精确可信构建器摘要与全树重新测量校验运行时缓存；仅未命中时才进入依赖缓存和原有离线构建单元。所有证据均为规范 ASCII JSON、`root:root 0444`、单硬链接，并以禁止跟随软链接的方式读取；异常条目先原子移动到固定隔离区再重建。若发布被强制终止或主机重启，下一轮也会隔离以发布名命名的 staging，但必须先证明对应拉取单元已停止；未知名称、活动单元、不安全控制根或目标歧义都会让发布失败关闭且不删除内容。缓存复用仍生成原有暂存运行时，不绕过安装、解析器自检、生产验收或回滚。
...
3. Upload all runtime, browser, test, documentation, and evidence files only to `/var/lib/turingmarket-gate/releases/v060-crm-sales-workspace-<timestamp>`, then persist and verify the immutable upload manifest. Prepare the exact parser runtime in the root-only lifecycle appliance: reuse a fully remeasured runtime cache when its trusted inputs and complete build evidence match, otherwise validate or rebuild the dependency cache and run the existing unprivileged offline build unit, including RapidOCR initialization. Bind the staged tree identity to the release manifest in either path. After installation, run the 21 self-tests inside `RootDirectory`, including XLSX, PPTX, and OCR inference. / 只向外置候选目录上传运行时、浏览器、测试、文档和证据文件，并持久化、验签不可变上传清单；随后在 root-only 生命周期设备中准备精确解析器运行时：可信输入和完整构建证据匹配时复用经全树重新测量的运行时缓存，否则校验或重建依赖缓存并运行原有非特权离线构建单元（含 RapidOCR 初始化）。两条路径都必须把暂存树身份绑定到发布清单。安装后在 `RootDirectory` 内执行包含 XLSX、PPTX 与 OCR 推理在内的 21 项自检。
...
The per-feature candidate Node gate is bounded to migration/replay evidence. Affected unit/API/contract tests run locally before deployment; the full non-browser aggregate runs at phase closeout or when a hard-gate risk boundary requires it. / 单功能候选 Node 门禁限定为迁移/重放证据；受影响的单元、API 与合同测试在部署前本地执行，完整非浏览器回归只在阶段收口或硬门禁风险边界触发时执行：

Production route smoke covers `/api/health`, `/m0`, `/m0-detail`, `/m4`, and `/admin` before the feature is accepted. Browser verification is not part of the per-feature gate unless the user explicitly selects it; it remains a phase-closeout check. / 单功能验收前，生产路由冒烟覆盖上述健康与页面路径；浏览器验证只有在用户明确选择时才进入单功能门禁，否则保留到阶段收口。
...
process.stdin.on("end", () => {
  const health = JSON.parse(body);
  const expected = "42e97a3ceec5df88ee1a11b9cd63314ed923b2a562a1121e7b8bd5f98f803c13";
...
Check the installed appliance identity and spool without following links. Expected runtime identity is 3,476 files, 435 directories, 640,588,717 bytes, SHA-256 `24905b29e26fe68791a32e931d92322a2ee87df0ae08baded7dec70874455cba`; the jobs directory must be empty outside active admissions. / 不跟随软链接检查已安装设备身份与 spool；预期运行时身份如前，非活动 admission 期间 jobs 目录必须为空。
...
## Current v0.7 Incremental Gates / 当前 v0.7 增量门禁

Production currently runs the accepted v0.6 shell and frozen PPT renderer plus reviewed v0.7 Phase 6 slices through Task 4D4 on schema v7. Historical full-regression and review results remain phase-closeout evidence, not proof for newer bytes. Task 4E is the next incremental candidate and must pass its focused tests, independent review, GitHub synchronization, verified backup, guarded deployment, and production acceptance before release. Browser and full-matrix gates run at phase closeout or when the risk boundary requires them. / 生产当前为已验收 v0.6 壳层与冻结 PPT renderer，叠加至 Task 4D4 的 v0.7 阶段 6 切片并运行 schema v7；历史完整回归和审查结果只作为阶段收口证据。下一候选 Task 4E 必须完成定向测试、独立审查、GitHub 同步、可验证备份、受控部署和线上验收后才可发布；浏览器和完整矩阵门禁保留到阶段收口或风险边界触发。
