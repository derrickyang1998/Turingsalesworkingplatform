...
5. As root, reconstruct an exact schema-only database from the consistent production backup, verify its fingerprint, and insert fixed synthetic sentinels for authentication, CRM, influencer, knowledge, and AI conversation data. The no-login `turingmarket-gate` user can read only this sanitized database, then runs the candidate migration offline and proves both the fingerprint and every sentinel remain intact before the complete Node suite, native Ubuntu 26 Playwright 1.61.1 Chromium-sandbox smoke, and writable-prefix Nginx validation. No production business row, user credential, or session token is exposed to candidate code. The frozen Playwright 1.60.0 Windows baseline remains unchanged. / root 从一致性生产备份中仅重建完全一致的数据库结构并校验指纹，再写入覆盖认证、CRM、网红、知识库和 AI 对话的固定合成哨兵。禁止登录的 `turingmarket-gate` 用户只能读取该脱敏数据库，在断网环境执行候选迁移，并证明结构指纹与全部哨兵保持完整，随后执行完整 Node、Ubuntu 26 原生 Playwright 1.61.1 Chromium 沙箱冒烟和可写前缀 Nginx 校验。候选代码不会接触任何生产业务行、用户凭据或会话令牌；冻结的 Playwright 1.60.0 Windows 基线保持不变。
