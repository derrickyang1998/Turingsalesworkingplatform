...
- Current focused slice / 当前定向切片：Task 4D3 governed knowledge lifecycle is production accepted. Schema v7 backfilled `123/123` knowledge governance rows; focused governance, parser, trusted-source, syntax, migration, backup, and online authorization/retrieval checks pass, and both independent reviews are `APPROVE`. The exact evidence is recorded in the Task 4D3 version record. / Task 4D3 可治理知识生命周期已通过生产验收；schema v7 完成 `123/123` 知识治理回填，知识治理、解析器、可信来源、语法、迁移、备份及线上权限/检索验证均通过，两次独立复审均为 `APPROVE`，精确证据见本切片版本记录。
...
- Production / 生产：accepted v0.6 shell plus v0.7 Phase 6 slices 1 through 4D3; App and PPT build markers remain unchanged / 已验收 v0.6 壳层叠加 v0.7 阶段 6 切片 1 至 4D3，App 与 PPT build 标识不变
...
Production currently runs the accepted v0.6 shell and frozen PPT renderer plus reviewed v0.7 Phase 6 slices through Task 4D3 on schema v7. Historical full-regression and review results remain phase-closeout evidence, not proof for newer bytes. Under the incremental cadence, Task 4D4 remains blocked only until its exact focused tests and independent review pass, the authoritative checkout and GitHub commit are synchronized, a verified backup and capacity record exist, and guarded deployment plus production health/login/core-path/rollback acceptance succeed. Browser and full-matrix gates run at phase closeout or when the risk boundary requires them. / 生产当前为已验收 v0.6 壳层与冻结 PPT renderer，叠加至 Task 4D3 的 v0.7 阶段 6 切片并运行 schema v7；历史完整回归和审查结果只作为阶段收口证据。Task 4D4 在最终定向测试与独立审查、权威工作区和 GitHub 同步、可验证备份与容量、受控部署及生产健康/登录/核心路径/回滚验收通过后即可上线；浏览器和完整矩阵门禁保留到阶段收口或风险边界触发。
