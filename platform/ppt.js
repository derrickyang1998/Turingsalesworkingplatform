...
function buildRevealHTML(data) {
  return buildCampaignReportHTML(data);
}
...
function buildCampaignReportHTML(data) {
  ...
  h += '<div class="page-controls"><button type="button" onclick="tmDeckPrev()">↑</button><span id="tmDeckCounter">1 / ' + (sections.length + 2 + (materials.length ? 1 : 0)) + '</span><button type="button" onclick="tmDeckNext()">↓</button></div><div class="page-progress" id="tmDeckProgress"></div>\n';
  h += '<script>\n' + campaignReportScript() + '\n<\/script>\n</body>\n</html>';
  return h;
}

function prepareCampaignReportData(data) {
  data = data || {};
  ...
  if (research && ((research.bullets || []).length || (research.sources || []).length)) {
    ...
    if (!sections.some(function(sec) { return sec.type === "research" || /联网调研|市场信号|调研/.test(sec.title || ""); })) {
      sections.splice(Math.min(2, sections.length), 0, {
        title: "联网调研与市场信号",
        type: "research",
        note: "生成前已联网搜索目标市场、竞品、平台内容和消费者表达，并用于校准本次策略。",
        points: buildPPTResearchPoints(research)
      });
    }
    if ((research.sources || []).length && !sections.some(function(sec) { return sec.type === "sources" || /调研来源|引用口径|来源/.test(sec.title || ""); })) {
      sections.push({
        title: "调研来源与引用口径",
        type: "sources",
        note: "来源用于策略判断参考，最终执行前建议二次核验。",
        points: research.sources.slice(0, 8).map(function(item, idx) {
          return "来源" + (idx + 1) + ": " + (item.title || item.url) + " | " + (item.snippet || item.url);
        })
      });
  ...
  return Object.assign({}, data, { sections: sections });
}

function campaignReportCSS() {
  return [
    ':root{--accent:#4f46e5;--accent-light:#7c3aed;--accent-soft:#eef2ff;--bg:#f7f8fb;--paper:#ffffff;--card:#ffffff;--line:#e5e7eb;--text:#111827;--text-body:#374151;--text-muted:#6b7280;--warm:#fff7ed;--mint:#ecfdf5;--font:Inter,-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif}',
    '*{margin:0;padding:0;box-sizing:border-box}html{height:100%;scroll-behavior:smooth;scroll-snap-type:y mandatory}body{min-height:100%;background:radial-gradient(circle at 12% 12%,rgba(79,70,229,.09),transparent 30%),radial-gradient(circle at 88% 18%,rgba(16,185,129,.08),transparent 28%),linear-gradient(180deg,#fff 0%,var(--bg) 100%);color:var(--text-body);font-family:var(--font);font-size:15px;line-height:1.6;overflow-x:hidden}body:before{content:"";position:fixed;inset:0;background-image:linear-gradient(rgba(17,24,39,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(17,24,39,.035) 1px,transparent 1px);background-size:42px 42px;mask-image:linear-gradient(180deg,rgba(0,0,0,.65),transparent 72%);pointer-events:none;z-index:0}',
    'nav{position:fixed;top:18px;left:28px;right:28px;z-index:1000;background:rgba(255,255,255,.82);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);border:1px solid rgba(229,231,235,.92);box-shadow:0 18px 45px rgba(15,23,42,.08);display:flex;align-items:center;justify-content:space-between;padding:0 18px;height:54px;border-radius:18px}.nav-brand{font-size:13px;font-weight:800;letter-spacing:.10em;color:var(--text-muted);white-space:nowrap}.nav-brand span{color:var(--accent)}.nav-links{display:flex;gap:4px;max-width:58vw;overflow:hidden}.nav-links a{font-size:11px;font-weight:700;letter-spacing:.06em;color:var(--text-muted);text-decoration:none;padding:6px 10px;border-radius:999px;transition:all .2s;white-space:nowrap}.nav-links a:hover{color:var(--text);background:var(--accent-soft)}.nav-links a.active{color:var(--accent);background:var(--accent-soft)}',
    'section{min-height:100vh;scroll-snap-align:start;padding:112px 72px 88px;display:flex;flex-direction:column;justify-content:center;position:relative;overflow:hidden;z-index:1}section:nth-child(even){background:rgba(255,255,255,.52)}.section-num{font-size:11px;font-weight:900;letter-spacing:.18em;color:var(--accent);text-transform:uppercase;margin-bottom:10px}.section-title{font-size:clamp(30px,4vw,54px);font-weight:900;color:var(--text);line-height:1.08;margin-bottom:12px;letter-spacing:-.035em;max-width:1080px}.section-sub{font-size:15px;color:var(--text-muted);margin-bottom:36px;max-width:780px}.divider{width:68px;height:4px;border-radius:999px;background:linear-gradient(90deg,var(--accent),var(--accent-light));margin-bottom:28px}',
    '.fade-in{opacity:0;transform:translateY(26px);transition:opacity .62s ease,transform .62s ease}.fade-in.visible{opacity:1;transform:translateY(0)}.delay-1{transition-delay:.08s}.delay-2{transition-delay:.16s}.delay-3{transition-delay:.24s}.delay-4{transition-delay:.32s}.card{background:rgba(255,255,255,.88);border:1px solid var(--line);border-radius:18px;padding:28px 32px;box-shadow:0 18px 50px rgba(15,23,42,.07)}.accent{color:var(--accent)}.accent-grad{background:linear-gradient(90deg,var(--accent),var(--accent-light));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}',
    '#cover{background:radial-gradient(ellipse 65% 55% at 50% 42%,rgba(79,70,229,.11) 0%,transparent 72%),linear-gradient(180deg,#fff 0%,#f8fafc 100%);align-items:center;justify-content:center;text-align:center;padding:128px 44px 90px}.cover-eyebrow{font-size:11px;font-weight:800;letter-spacing:.24em;color:var(--text-muted);text-transform:uppercase;margin-bottom:30px}.cover-title{font-size:clamp(38px,6vw,74px);font-weight:950;color:var(--text);line-height:1.04;margin-bottom:22px;max-width:1120px;letter-spacing:-.05em}.cover-sub{font-size:clamp(15px,2vw,20px);color:var(--text-muted);margin-bottom:44px;max-width:780px;margin-left:auto;margin-right:auto}.chips{display:flex;flex-wrap:wrap;gap:12px;justify-content:center;margin-bottom:46px}.chip{background:#fff;border:1px solid rgba(79,70,229,.22);box-shadow:0 12px 32px rgba(79,70,229,.08);color:var(--accent);font-size:12px;font-weight:800;padding:9px 18px;border-radius:999px;letter-spacing:.04em}.cover-footer{font-size:12px;color:var(--text-muted)}.orb{position:absolute;border-radius:50%;pointer-events:none;z-index:0;filter:blur(70px)}.orb-1{width:520px;height:520px;background:rgba(79,70,229,.08);top:-120px;right:-130px}',
    '.summary-grid,.insight-grid,.game-grid,.kpi-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(270px,1fr));gap:20px}.summary-card{transition:border-color .25s,transform .25s,box-shadow .25s}.summary-card:hover{border-color:rgba(79,70,229,.28);transform:translateY(-4px);box-shadow:0 26px 60px rgba(79,70,229,.12)}.summary-num{font-size:11px;font-weight:900;letter-spacing:.14em;color:var(--accent);margin-bottom:10px}.summary-card h3,.insight-card h3,.game-card h3{font-size:17px;font-weight:850;color:var(--text);margin-bottom:10px}.summary-card p,.insight-card p,.game-card p{font-size:13.5px;color:var(--text-muted);line-height:1.72}.insight-icon{font-size:28px;margin-bottom:14px}',
    '.stat-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:20px}.stat-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:28px 24px;position:relative;overflow:hidden;box-shadow:0 18px 45px rgba(15,23,42,.06)}.stat-card:before{content:"";position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,var(--accent),var(--accent-light))}.stat-num{font-size:clamp(30px,3.5vw,48px);font-weight:950;color:var(--accent);line-height:1;margin-bottom:10px}.stat-label{font-size:13px;color:var(--text-muted);line-height:1.45}',
    '.wave-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:24px}.wave-card{background:#fff;border:1px solid var(--line);border-radius:22px;overflow:hidden;transition:transform .25s;box-shadow:0 18px 48px rgba(15,23,42,.07)}.wave-card:hover{transform:translateY(-4px)}.wave-header{padding:20px 28px 16px;background:linear-gradient(135deg,#eef2ff,#f5f3ff)}.wave-num{font-size:10px;font-weight:900;letter-spacing:.18em;color:var(--accent)}.wave-title{font-size:18px;font-weight:900;color:var(--text);margin-top:5px}.wave-period{font-size:12px;color:var(--text-muted);font-weight:700;margin-top:2px}.wave-body{padding:24px 28px 28px}.wave-theme{font-size:13px;font-weight:800;color:var(--text);margin-bottom:12px}.wave-actions{font-size:13px;color:var(--text-muted);line-height:1.72}.wave-kpi-row{display:flex;gap:8px;flex-wrap:wrap;margin-top:16px}.wave-kpi{background:var(--bg);border:1px solid var(--line);border-radius:999px;padding:6px 12px;font-size:11px;color:var(--text-muted);font-weight:700}',
    '.platform-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px}.platform-card{background:#fff;border:1px solid var(--line);border-radius:16px;padding:22px 20px;box-shadow:0 14px 40px rgba(15,23,42,.06)}.platform-name{font-size:14px;font-weight:850;color:var(--text);margin-bottom:6px}.platform-pct{font-size:30px;font-weight:950;color:var(--accent);margin-bottom:8px}.platform-bar-track,.wb-track{height:8px;background:#eef2ff;border-radius:999px;overflow:hidden}.platform-bar-fill,.wb-fill{height:100%;border-radius:999px;background:linear-gradient(90deg,var(--accent),var(--accent-light));width:0;transition:width 1.1s cubic-bezier(.4,0,.2,1)}.platform-desc{font-size:12px;color:var(--text-muted);margin-top:9px}',
    '.budget-table{width:100%;border-collapse:collapse}.budget-table th{font-size:11px;font-weight:800;letter-spacing:.08em;color:var(--text-muted);text-align:left;padding:8px 12px;border-bottom:1px solid var(--line)}.budget-table td{font-size:13px;color:var(--text-body);padding:12px;border-bottom:1px solid #f1f5f9}.budget-table tr:hover td{background:#f8fafc}',
    '.gantt-wrap{overflow-x:auto}.gantt{min-width:780px;display:flex;flex-direction:column;gap:10px}.gantt-row{display:grid;grid-template-columns:170px 1fr 250px;gap:10px;align-items:stretch}.gantt-label{font-size:12px;font-weight:850;color:var(--text);display:flex;align-items:center;padding:12px;background:#fff;border:1px solid var(--line);border-radius:10px}.gantt-block{border-radius:10px;min-height:48px;display:flex;align-items:center;padding:10px 14px;background:linear-gradient(135deg,#eef2ff,#f5f3ff);border:1px solid rgba(79,70,229,.18);font-size:12px;font-weight:750;color:var(--text)}.gantt-output{font-size:12px;color:var(--text-muted);display:flex;align-items:center;padding:10px 12px;background:#fff;border:1px solid var(--line);border-radius:10px}',
    '.kpi-card{background:#fff;border:1px solid var(--line);border-radius:18px;padding:28px;box-shadow:0 16px 44px rgba(15,23,42,.06)}.kpi-card-header{display:flex;align-items:center;gap:10px;margin-bottom:16px}.kpi-icon{width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#eef2ff,#f5f3ff);border:1px solid rgba(79,70,229,.18);display:flex;align-items:center;justify-content:center;font-size:16px;color:var(--accent);font-weight:900}.kpi-card-title{font-size:14px;font-weight:850;color:var(--text)}.kpi-item{display:flex;align-items:flex-start;gap:8px;margin-bottom:10px}.kpi-dot{width:6px;height:6px;border-radius:50%;background:var(--accent);margin-top:7px;flex-shrink:0}.kpi-text{font-size:13px;color:var(--text-muted);line-height:1.55}',
    '.material-card{background:#fff;border:1px solid rgba(79,70,229,.18);border-radius:18px;padding:22px;box-shadow:0 16px 44px rgba(15,23,42,.06)}.material-head{display:flex;justify-content:space-between;gap:16px;align-items:center;margin-bottom:10px}.material-title{font-size:15px;font-weight:850;color:var(--text)}.material-state{font-size:11px;color:var(--accent);background:var(--accent-soft);border-radius:999px;padding:4px 10px;white-space:nowrap;font-weight:800}.material-preview{font-size:13px;color:var(--text-muted);line-height:1.65;max-height:180px;overflow:hidden}.highlight-box{background:#fff;border:1px solid rgba(79,70,229,.18);border-radius:16px;padding:20px 24px;margin-top:24px;box-shadow:0 16px 44px rgba(15,23,42,.05)}.highlight-box p{font-size:13.5px;color:var(--text-body);line-height:1.65}.confidential{position:fixed;bottom:16px;right:20px;font-size:10px;color:var(--text-muted);opacity:.55;letter-spacing:.08em;z-index:999}.source-note{font-size:11px;color:var(--text-muted);border-top:1px solid var(--line);padding-top:16px;margin-top:32px;opacity:.8}',
    '#closing{background:radial-gradient(ellipse 70% 60% at 50% 48%,rgba(79,70,229,.10) 0%,transparent 70%),linear-gradient(180deg,#fff 0%,#f8fafc 100%);text-align:center;align-items:center;justify-content:center}.closing-title{font-size:clamp(32px,5vw,62px);font-weight:950;color:var(--text);line-height:1.12;margin-bottom:20px;max-width:960px}.closing-sub{font-size:18px;color:var(--text-muted);margin-bottom:42px}.closing-footer{font-size:13px;color:var(--text-muted)}.page-controls{position:fixed;right:22px;top:50%;transform:translateY(-50%);z-index:1001;display:flex;flex-direction:column;align-items:center;gap:9px;background:rgba(255,255,255,.84);border:1px solid var(--line);box-shadow:0 18px 42px rgba(15,23,42,.10);border-radius:999px;padding:10px 8px}.page-controls button{width:32px;height:32px;border:0;border-radius:50%;background:var(--accent);color:#fff;font-weight:900;cursor:pointer}.page-controls span{writing-mode:vertical-rl;font-size:11px;color:var(--text-muted);font-weight:800;letter-spacing:.04em}.page-progress{position:fixed;left:0;top:0;height:4px;background:linear-gradient(90deg,var(--accent),var(--accent-light));width:0;z-index:1002;transition:width .2s ease}.cursor-orb{position:fixed;width:220px;height:220px;border-radius:50%;pointer-events:none;z-index:0;background:radial-gradient(circle,rgba(79,70,229,.11),transparent 65%);filter:blur(10px);transform:translate(-50%,-50%);transition:opacity .2s;opacity:.85}',
    '@media(max-width:768px){html{scroll-snap-type:none}section{padding:88px 24px 62px}nav{top:10px;left:10px;right:10px;padding:0 14px}.nav-links,.page-controls,.confidential,.cursor-orb{display:none}.gantt-row{grid-template-columns:1fr}.section-title{font-size:30px}}@media print{html{scroll-snap-type:none}nav,.confidential,.page-controls,.page-progress,.cursor-orb{display:none}section{min-height:auto;page-break-after:always;padding:48px 36px}.fade-in{opacity:1!important;transform:none!important}}'
  ].join('\n');
}
...
function renderCampaignSection(sec, idx, allSections) {
  ...
  else if (/排期|timeline|里程碑/i.test(sec.title || "")) h += renderTimeline(points);
...
function campaignReportScript() {
  return 'const fadeEls=[...document.querySelectorAll(".fade-in")];const fadeObserver=new IntersectionObserver((entries)=>{entries.forEach(e=>{if(e.isIntersecting)e.target.classList.add("visible")})},{threshold:.12});fadeEls.forEach(el=>fadeObserver.observe(el));document.querySelectorAll(".platform-bar-fill[data-width],.wb-fill[data-width]").forEach(el=>{new IntersectionObserver((entries)=>{entries.forEach(e=>{if(e.isIntersecting)e.target.style.width=e.target.dataset.width+"%"})},{threshold:.3}).observe(el)});const sections=[...document.querySelectorAll("section[id]")];const navLinks=[...document.querySelectorAll(".nav-links a")];let activeIndex=0;function updateDeckState(i){activeIndex=Math.max(0,Math.min(sections.length-1,i));const id=sections[activeIndex]?.id;navLinks.forEach(a=>{a.classList.toggle("active",(a.getAttribute("href")||"").replace("#","")===id)});const c=document.getElementById("tmDeckCounter");if(c)c.textContent=(activeIndex+1)+" / "+sections.length;const p=document.getElementById("tmDeckProgress");if(p)p.style.width=((activeIndex+1)/sections.length*100)+"%"}function scrollToSection(i){const target=sections[Math.max(0,Math.min(sections.length-1,i))];if(target)target.scrollIntoView({behavior:"smooth",block:"start"})}window.tmDeckNext=()=>scrollToSection(activeIndex+1);window.tmDeckPrev=()=>scrollToSection(activeIndex-1);const navObserver=new IntersectionObserver((entries)=>{entries.forEach(e=>{if(e.isIntersecting){updateDeckState(sections.indexOf(e.target))}})},{threshold:.58});sections.forEach(s=>navObserver.observe(s));addEventListener("keydown",e=>{if(["ArrowDown","PageDown"," "].includes(e.key)){e.preventDefault();tmDeckNext()}if(["ArrowUp","PageUp"].includes(e.key)){e.preventDefault();tmDeckPrev()}if(e.key==="Home"){e.preventDefault();scrollToSection(0)}if(e.key==="End"){e.preventDefault();scrollToSection(sections.length-1)}});const orb=document.getElementById("cursorOrb");addEventListener("pointermove",e=>{if(orb){orb.style.left=e.clientX+"px";orb.style.top=e.clientY+"px"}});updateDeckState(0);';
}
