...
### 策略生成审计链 / Auditable Strategy Generation
- `/api/ai/strategy` 现在把 `prompt` 与业务输入完整送入统一知识优先 AI 服务，并将同一上下文用于权限过滤后的知识检索和可选联网查询；对话、消息、知识/联网引用、Token 与摘要均可审计。
- 保留联网默认开启的兼容行为，并支持 `allow_web=false` 显式关闭；团队摘要仍为默认值，同时允许显式覆盖为个人私有。模型降级会映射到既有 `fallback`，安全原因通过新增 `warning` 返回。
- `knowledge_limit` 只接受 `1-100` 的整数或纯数字字符串；缺失、越界、布尔、数组、对象和带空格字符串均回退到 8，不能用隐式类型转换绕过知识优先合同。

### 方案草稿与人工确认闭环 / Proposal Draft And Human Confirmation Loop
- M3 方案生成改为调用 `/api/ai/proposal-draft`：需求和模板用于权限过滤后的知识检索，生成指令不污染检索词；联网默认关闭，AI 对话、消息、知识引用、Token 和个人私有摘要进入统一审计链。
