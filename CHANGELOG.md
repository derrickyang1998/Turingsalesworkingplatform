...
## 商务 SOP v2.0.2 培训整合版（2026-09-17）

- 以用户修订的 `商务团队客户全生命周期与日常作业 SOP v2.0.1` 为母版，新建 `商务团队客户全生命周期与日常作业SOP-v2.0.2（培训整合版）.md`，原始下载文件保持不变。
- 将 `商务培训/00—07` 接入 SOP：五周培养、理论与红线、平台案例、五账号达人实操、Roleplay、项目答辩、跟会和主管陪同要求均形成明确门槛。
- 补齐平台方案的 14 项核验字段：平台角色与特性、受众、发现机制、内容、合作形式与要求、链接、转化路径、付费放大、指标、适配、风险和事实来源。
- 明确 Sales 负责证据化一轮初筛，Ops 负责达人真实性、价格、档期、授权、品牌安全和交付可行性的最终专业复核；未复核不得锁定或启动执行。
- 保留 v2.0.1 的终止结算流程、内部业务链接、转正和提成数字；对“下单、回款、实际消耗、毛利成本范围”尚未统一的口径作显性提示，不擅自改动制度数字。
- 修复失效目录、标题转义、空标题、模板章节引用、指标分母和 ROAS/ROI 口径；保留 v2.0.1 的客户成功、预付款和 30 分钟需求会口径，并把其内部矛盾列为管理层待确认项，不在培训整合中擅自改写。
- 培训索引、基础培训手册和题库的当前 SOP 链接同步到 v2.0.2；v2.1 保留为培训设计来源。

## 商务培训体系 v1.0 与客户全生命周期 SOP v2.1（2026-09-08）

- 新增受控商务培训包 `00—07`：覆盖红人营销基础、12 个主流社媒平台与商业化路径、8 类产品匹配案例、达人质量评分卡、48 道知识题、6 个行业 Roleplay 及五周上岗认证。
- 将培训门禁接入 `商务团队客户全生命周期与日常作业SOP-v2.1.md`，明确理论、红线、平台案例、达人实操、Roleplay、陪访、主管陪同和交接演练要求。
- 明确商务负责达人初筛、推荐逻辑和重大风险识别，红人运营负责最终专业复核与执行可行性确认；平台动态事实统一由来源维护台账按季度及重大变化复核。
- v2.1 继续保持“受控试运行版”；系统字段、审批、Owner 与团队认证未全部验收前不转为正式生效，且流程生效不代表薪酬、转正或提成制度生效。
- 保留 SOP v2.0 与历史 Roleplay 原文件作为只读基线，不覆盖旧版本。
